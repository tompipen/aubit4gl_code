...
define
   ...
   drep_def array[20] of record  # Array de descritores de campos
      ...
      coluna      smallint,      # Coluna relativa em que vai ficar.
		...
		total_de   float,          # Campo utilizado para totalizacoes.
   ...
   n_linhas       smallint,     # Numero de linhas da tabela. Para d_count
...
{}
   ...
   if n_linhas <= 0 or n_linhas is null or status = notfound then
      call d_dialogOk(9,15,"","Listagem pedida",
                              "nao tem dados","para paremtros escolhidos",
                              "!","")
      return false
   end if
   ...
   let str_sel = "select ", str_fields clipped, " from ", 
                 str_tabelas clipped, " where ", 
                 str_where clipped, " order by ", str_order clipped
   ...
   define 
	...
	for i = 1 to ncolunas
      ...
      case drep_def[i].tipo
         ...
         WHEN 3  # Float. Meter a ir buscar formatos default, etc.
				if drep_def[i].tamanho is null then
...
{}
	case 
		when max_len <= 85
			...
			let tipo_font  = 10
			let tam_risco  = 84
		when max_len > 85 and max_len <= 100
			let tipo_papel = "8IN"
			let tipo_font  = 12
			let tam_risco  = 105
		when max_len > 100 and max_len <= 144
			let tipo_papel = "8IN"
			let tipo_font  = 17
			let tam_risco  = 144
...
{}
   ...
   return drep_def[coluna_curr].coluna, str_report
end function
...
{}
   ...
   case tipo  # Meter a entrar em linha de conta com o tamanho.
      WHEN 0  # Char
         let str_report = y[coluna_curr]
      WHEN 1  # Smallint
         let um_int = y[coluna_curr]
         let str_report = um_int using formato
      WHEN 2  # Integer
         let um_int = y[coluna_curr]
         let str_report = um_int using formato
      WHEN 3  # Float. Meter a ir buscar formatos default, etc.
         let um_float = y[coluna_curr]
         let str_report = um_float using formato
      WHEN 4  # Smallfloat.
         let um_float = y[coluna_curr]
         let str_report = um_float using formato
      WHEN 5  # Decimal.
...
{}
   ...
   case tipo  # Meter a entrar em linha de conta com o tamanho.
      ...
      WHEN 3  # Float. Meter a ir buscar formatos default, etc.
            ...
            let str_report = drep_def[coluna_curr].total_de using formato
			end if
      WHEN 4  # Smallfloat.
			if drep_def[coluna_curr].totalizacao = "S" then
            let str_report = drep_def[coluna_curr].total_de using formato
