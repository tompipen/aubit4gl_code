...
database sysmaster
	...
	define strExtenso char(256)

{
# bloco de dbug, depois apagar

database despodata

main
	define num decimal(32,16)
	define numaux decimal(32,16)
	define tamstr smallint
	define moeda_std char(10)
	define tmp_extenso char(500)

		let num=arg_val(1)
		let tamstr=arg_val(2)
		let moeda_std=arg_val(3)
		...
		let numaux=num*1000
			...
			call extenso(num,tamstr,moeda_std)
			returning tmp_extenso
			display tmp_extenso
...
# Acrescentei
	...
	if mil_milhoes > 0 then
			if mil_milhoes = 1 then
					...
					let strExtenso = strExtenso clipped, numero(mil_milhoes, flag+1)
 ...
 # Milhoes	
			if milhoes > 0  then#and FezMilMilhoes != "S") then
					if milhoes = 1 then
						...
						let strExtenso = strExtenso clipped, numero(milhoes,flag+1)
 ...
 # Milhares
			if milhares > 0 then
					if milhares =1 then
						...
						let strExtenso = strExtenso clipped, numero(milhares,flag+1)
						let strExtenso = strExtenso clipped, nome[5]
					end if
					let FezMilhares = "S"
								if unidades > 0 then
	...
	# Unidades
		if unidades > 0  then 
				let strExtenso = strExtenso clipped, numero(unidades,flag+1)
		end if
	...
	END IF
		...
		if centavos > 0 then
				let strExtenso = strExtenso clipped, " e "
				let strExtenso = strExtenso clipped, numero(centavos,flag+1)
