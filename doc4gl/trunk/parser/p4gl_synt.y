...
menu_control
  : BEFORE MENU op_fgl_statement_list
...
set_clause_ansi
  ...
  | column_specification '=' '(' query_specification ')' 
                   {$$=CpStr("%s=(%s)",$1,$4);}
...
subquery:
  ...
  '(' SELECT op_ad select_list table_expression ')'
...
exists_predicate
  : op_not EXISTS subquery  { $$=CpStr("EXISTS %s","Subquery"); }
  ;
