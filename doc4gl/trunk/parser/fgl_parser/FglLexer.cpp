...
/**
 * @todo : This should be an object and a pointer in the Lexer object
 */
 ...
 */
void FglLexer::lexer_ungetc (int a)
{
  memFile->ungetchar(a);
 ...
 * @param instr
 */
void FglLexer::ccat (char *s, char a, int instr)
{
  char buff[3];
  if (strlen (s) >= 1023)
  {
 ...
 *    1 -
 */
int FglLexer::words (int cnt, int pos,char *p)
{
  int z;
  int t;
  char buff[132];
 ...
 */
int FglLexer::get_hash_val (char *s)
{
  int c;
  c = toupper (s[0]);
  if (c >= 'A' && c <= 'Z')
 ...
 * @param arr
 */
void FglLexer::turn_state_all (int kw, int v, int arr)
{
 ...
 * @param v
 */
void FglLexer::turn_state (int kw, int v)
{
  int a;
  for (a = 0; a <= 26; a++)
  {
...
/** 
 * @todo : Fix this way of use the lexer. This way we can have only one lexer 
 * at each time and what we should have any that we can
 */
extern FglLexer *fglLexer;

/*
int a4gl_yylex (void *pyylval, int yystate, void *yys1, void *yys2) 
{
  fglLexer->yylex(pyylval,yystate,yys1,yys2);
}
*/

void turn_state (int kw, int v)
{
  fglLexer->turn_state(kw, v);
}
