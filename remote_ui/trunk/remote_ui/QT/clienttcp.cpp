...
bool ProtocolHandler::startReportTemplate(QString odffile, QString sedfile)
{
   ...
   if(sedfile.contains("SCAN:")) {
           ...
           if(!file->open(QIODevice::ReadOnly)) {
               qDebug() << "konnte datei nicht �ffnen" << "";
               //qWarning(QString("Konnte Datei nicht zum lesen �ffnen: %1").arg(templateFile).toLatin1());
               return false;
           }
   ...
   //�berpr�fen der SED Datei wie viele eintr�ge der ersten Variable von der 1ten Ebene vorhanden ist.
...
void ProtocolHandler::replaceTemplateVars(QString odffile, QString sedfile)
{
    ...
    if(!file1->open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        qDebug() << "konnte nicht zum schreiben �ffnen (ersetzen)" << "";
    }
                ...
                if(ausgabe.contains(fieldlist.at(i))) {
                    ...
                    if(checkSedFile(fieldlist.at(i), sedfile)) {
                        ...
                        if(!seddatei->open(QIODevice::ReadOnly)) {
                            qDebug() << "konnte SED Datei nicht zum lesen �ffnen (ersetzen)" << "";
                        }
