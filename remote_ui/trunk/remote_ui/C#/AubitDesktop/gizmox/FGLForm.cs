        ...
        void addLayoutToParentForXmlForm(Control parent, object child)
        {
                ...
                case "AubitDesktop.Xml.XMLForm.VBox":
                    {
                        ...
                        if (p.pixelWidth > 0 && p.pixelHeight > 0)
                        {
                            thisLayoutControlsPanel.AutoSize = false;
                            thisLayoutControlsPanel.Width = p.pixelWidth;
                            thisLayoutControlsPanel.Height = p.pixelHeight;
                        }
 			...
 			if ((p.width != null && p.width != "") || (p.height!=null && p.height!=""))
                        ...
                        {              
                            ...
                            if (item.GetType() == typeof(Xml.XMLForm.CheckBox))
                            {
                                ...
                                dc=new Gizmox.WebGUI.Forms.DataGridViewCheckBoxColumn();
                            ...
                            if (item.GetType()==typeof(Xml.XMLForm.Edit)) {
                                ...
                                dt = new Gizmox.WebGUI.Forms.DataGridViewTextBoxColumn();
                            ...
                            if (!added)
                            {
                                gridArray.Columns.Add(new Gizmox.WebGUI.Forms.DataGridViewTextBoxColumn());
                                gridArray.Columns[a + 1].ValueType = typeof(string);
                            }
			    ...
			    double factor=1.0; 
			     factor=1.5; 
                        ...
                        gridArray.sizeGrid();
                        if (p.pixelHeight > 0 && p.pixelWidth > 0)
                        {
                            gridArray.Height = p.pixelHeight;
                            gridArray.Width = p.pixelWidth;
                        }
                ...
                case "AubitDesktop.Xml.XMLForm.Grid":
                    {
                        ...
                        else
                        {
                              ...
                              width = GuiLayout.get_gui_w(Convert.ToInt32(p.width));
                                ...
                                if (p.pixelHeight > 0 && p.pixelWidth > 0)
                                {
                                    thisScreensPanel.Height = p.pixelHeight;
                                    thisScreensPanel.Width = p.pixelWidth;
                                }
                        }
                       // thisScreensPanel.Height = GuiLayout.get_gui_h(height+1);
                        //thisScreensPanel.Width = GuiLayout.get_gui_w(width+1);
			...
			// p.ResumeLayout();
                ...
                //case "AubitDesktop.Xml.XMLForm.Page[]":
                   ...
                   //     int maxHeight = 0;
                   //     int maxWidth = 0;
                        AubitDesktop.Xml.XMLForm.Folder folder;
                        folder = (AubitDesktop.Xml.XMLForm.Folder)child;
                        ...
                        if (folder.pixelHeight == 0 || folder.pixelWidth == 0)
                        {
                            tb.Size = parent.Size;
                        ...
                        {
                            tb.Width = folder.pixelWidth;
                            tb.Height = folder.pixelHeight;
                        }
                        ...
                        //tb.Dock = DockStyle.Fill;
                        for (int a = 0; a < folder.Pages.Length; a++)
                        {
                            TabPage tp;
                            Panel tpp;
                            if (folder.Pages[a].text != null) { tp = new TabPage(folder.Pages[a].text); } else { tp = new TabPage("Page " + a); }
                            ...
                            tpp = new Panel();
                            tpp.Name = "Tpp" + folder.Pages.GetHashCode();
                            tpp.SuspendLayout();
                            tpp.AutoSize = true;
                  ...
                  //          tpp.BorderStyle = BorderStyle.FixedSingle;
                            ...
                            addLayoutToParentForXmlForm(tpp, folder.Pages[a]);
                ...
                case "AubitDesktop.Xml.XMLForm.HLine":
                    {
                       ...
                       // lineHeight=GuiLayout.get_gui_y(Convert.ToInt32(hl.posY+1))GuiLayout.get_gui_y(Convert.ToInt32(hl.posY))-GuiLayout.get_gui_y(Convert.ToInt32(hl.posY))
                        ...
                        p_for_line.BorderStyle = Gizmox.WebGUI.Forms.BorderStyle.Fixed3D;
            ...
            foreach (object o in f.SCREENS.SCREENLAYOUT)
            {
                ...
                if (o is FORMLABEL)
                {
                    ...
                    if (isJustLines(flabel.Text))
                    {
                        ...
                        p.BorderStyle = Gizmox.WebGUI.Forms.BorderStyle.Fixed3D;
