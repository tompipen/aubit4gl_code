...
MAIN
  DEFINE exitStatus SMALLINT
	...
	DEFINE lv_money MONEY

  LET exitStatus = 0

	INITIALIZE lv_int TO NULL
	LET lv_money = lv_int
	IF lv_money IS NOT NULL THEN
	   DISPLAY "The conversion from null integer to money was not null"
		 LET exitStatus = 1
	END IF

	LET lv_int = 123446
	LET lv_money = lv_int
	IF lv_money != 123446 OR lv_int IS NULL THEN
	...
	LET lv_money = lv_int
	IF lv_money != 2000000000 OR lv_int IS NULL THEN
	...
	LET lv_int = 0
	LET lv_money = lv_int
	IF lv_money != 0 OR lv_money IS NULL THEN
