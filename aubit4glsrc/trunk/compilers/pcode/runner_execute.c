...
static void add_block_to_stack (int pc,struct cmd_block *b) {
  int a;
...
static void copy_across_params (struct param *dest,struct param *src) {
...
void * get_var_ptr (struct use_variable *uv)
{
int a;
int found=-1;
struct variable *var;
...
for (a=0;a<callstack[call_stack_entry].c_vars.c_vars_len;a++) {
	...
	if (uv->variable_id==callstack[call_stack_entry].c_vars.c_vars_val[a].variable_id) {
		found=a;
		break;
	}
}
...
switch (var->category) {
        ...
        case CAT_NORMAL: 
        case CAT_EXTERN:
	case CAT_STATIC: fprintf(stderr,"Variable not allocated... %d\n",var->category); exit(2);

        case CAT_ALLOC		:      	pointer_or_offset='O'; break;
        case CAT_ALLOC_STATIC	:	pointer_or_offset='P'; break;
        case CAT_ALLOC_EXTERN	:	pointer_or_offset='P'; break;
        case CAT_SUBSTRUCT	: 	pointer_or_offset='O'; break;
        case CAT_PARAMETER	:	pointer_or_offset='O'; break;
        case CAT_PARAMETER_SET	:	pointer_or_offset='O'; break;
}
...
if (uv->sub.sub_len) { // Darn - we've got more to do yet...
	int a;
...
if (uv->indirection==-1) { return  &rptr; }
if (uv->indirection==1) { return  (void *)*(int *)rptr; }
if (rptr==0) {
	printf("Uninitialized : ");
	print_use_variable(uv);
}
if (uv->indirection==0) {return rptr;}
