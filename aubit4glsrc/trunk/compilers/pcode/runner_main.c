...
run_module ()
{
  ...
  if (this_module.fglc_version != FGLC_XDR_VERSION)
    {
      fprintf
	(stderr,"This module was compiled as pcode version %4.2f - this is a version %4.2f runner\n",
...
//
  ...
  main_params.param_u.p_list->list.list_val[0].param_type=PARAM_TYPE_LITERAL_INT;
...
find_pcode_function (char *s)
{
  ...
  for (a = 0; a < this_module.functions.functions_len; a++)
    {
      if (strcmp (GET_ID (this_module.functions.functions_val[a].func_name_id), s) == 0) { return a; }
    }
  return -1;
}
...
if (c->cmd_type!=CMD_BLOCK) {
	fprintf(stderr,"First thing in a function should always be a block...\n");
	exit(74);
}
...
if (p) {
	...
	if (p->param_type!=PARAM_TYPE_LIST) {
		...
		expecting=this_module.functions.functions_val[func_no].param_vars.param_vars_len;
		...
		for (a=0;a<got;a++) {
			...
			memcpy(sv_var,
				&this_module.functions.functions_val[func_no].param_vars.param_vars_val[a],sizeof(struct use_variable));
			set_var(0,&sv);
		}
	}
} else {
...
while (1) {
	...
	switch (c->cmd_type) {
		case CMD_NULL  		: pc++; break;
		case CMD_BLOCK 		: fprintf(stderr,"Should only be one block in a function and I should have already run that - my head hurt, i'm going to lie down\n");exit(88);
//execute_start_block(pc,c->cmd_u.c_block);pc++; break;
		case CMD_NOP 		: pc++; break; // This is a no-op - do nothing...
		case CMD_END_BLOCK	: execute_end_block();pc++; break;
		case CMD_CALL		: call_function(pc,c->cmd_u.c_call);pc++; break;
		case CMD_IF		: evaluate_param(c->cmd_u.c_if->condition,&i);
					  if (i) {
						pc=pc+c->cmd_u.c_if->goto_true;
						if (c->cmd_u.c_if->goto_true==0) pc++; 	// NOP GOTO
					  } else {
						pc=pc+c->cmd_u.c_if->goto_false;
						if (c->cmd_u.c_if->goto_false==0) pc++; 	// NOP GOTO
					  }
					  break;
		case CMD_SET_VAR	: set_var(pc,c->cmd_u.c_setvar); pc++; break;
		case CMD_SET_VAR_ONCE	: set_var_once(pc,c->cmd_u.c_setvar1); pc++; break;
		case CMD_GOTO_LABEL	: fprintf(stderr,"Unresolved goto remains...\n"); exit(1);
		case CMD_GOTO_PC	: pc=pc+c->cmd_u.c_goto_pc;
					  if (c->cmd_u.c_goto_pc==0) pc++; 		// NOP GOTO 
					  break;
		case CMD_RETURN		: 
