...
int main(int argc, char *argv[])
{
   ...
   if(hDoc == NULL) {
      MsgBox("Document must be open in editor, to return it to version controll !", MB_OK);
      return 1;
   }
   ...
   // from the filename and takeout.ini determine default filename and location
   {
      ...
      UnixPathToDosPath(szDocFileName);
       ...
       GetPrivateProfileString("Remote", "LastTakeOutLocation", "",
            g_szFileLocation, sizeof(g_szFileLocation), "gte.ini");


      if(szDocFileName)
         GetPrivateProfileString("TakeOutLocation", szDocFileName, g_szFileLocation,
            g_szFileLocation, sizeof(g_szFileLocation), "takeout.ini");
...
//LastTakeOutFileName=prof_main.4gl
      ...
      if(szDocFileName)
         GetPrivateProfileString("TakeOutFileName", szDocFileName, g_szFileName,
            g_szFileName, sizeof(g_szFileName), "takeout.ini");
...
void PutBack(void)
{
   ...
   // Check if this file is already PutBack
   {
      ...
      bPutBack = GetPrivateProfileInt("PutBack", szCmdLine, 0, "takeout.ini");
      if(bPutBack)
      {
         if(MsgBox("File is already in the source control. Are you want to put back file again?",
         ...
         if(MsgBox("File is already in the source control. Do you know what are you doing?",
            MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) != IDYES)
         {
            return;
         }
         if(MsgBox("LAST WARNING: Continue?",
            MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) != IDYES)
         {
            return;
         }
   ...
   GetPrivateProfileString("GWD Applications", "Text Editor","", g_GWDlocation, sizeof(g_GWDlocation), "gwd.ini");
