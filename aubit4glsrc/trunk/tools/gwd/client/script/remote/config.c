...
//   {
      ...
      switch(nIdx)
      {
         case 1: OpenTOC(); return(0);                 // break;
         case 2: return(0);                            // break;
         case 3: return(0);                            // break;
         case 4: return(0);                           // break;
         case 5: RemoteOptionsConfig(); return(0);     // break;
         case 6: MkDirFromFile();  return(0);          // break;
         case 7: TerminalIniFile(); return(0);
         case 8: test_4gl_version(); return(0);
         case 9: test_env_read(); return(0);
         case 10: test_time(); return(0);
		 //       ReadFile(HDOC);
...
void RemoteOptionsConfig(void)
{
   // read configuration from ini file
   GetPrivateProfileString("Remote", "TakeOutCmd", "", g_szTakeOutCmd, sizeof(g_szTakeOutCmd), "remote.ini");
   ...
   {
      WritePrivateProfileString("Remote", "TakeOutCmd", g_szTakeOutCmd, "remote.ini");
   }

   if(Gte_GetStringDlg("PutBack command", "&PutBack command:",
      g_szPutBackCmd, sizeof(g_szPutBackCmd)) != FALSE)
   {
      WritePrivateProfileString("Remote", "PutBackCmd", g_szPutBackCmd, "remote.ini");
   }
...
//      g_szPutBackCmd, sizeof(g_szMakeCmd)) != FALSE)
   ...
   {
      WritePrivateProfileString("Remote", "MakeCmd", g_szMakeCmd, "remote.ini");
   }

   if(Gte_GetStringDlg("Server Initial Directory", "&Server initial dir:",
      g_szServerInitDir, sizeof(g_szServerInitDir)) != FALSE)
   {
      WritePrivateProfileString("Remote", "ServerInitDir", g_szServerInitDir, "remote.ini");
   }
   ...
   if(Gte_BrowseForDirDlg(g_szClientInitDir, sizeof(g_szClientInitDir), FALSE) != FALSE)
      WritePrivateProfileString("Remote", "ClientInitDir", g_szClientInitDir, "remote.ini");

   if(Gte_GetStringDlg("Server Name", "&Server name:", g_szServerName, sizeof(g_szServerName)) != FALSE)
      WritePrivateProfileString("Remote", "ServerName", g_szServerName, "remote.ini");

   if(Gte_GetStringDlg("User Name", "&User name:", g_szUserName, sizeof(g_szUserName)) != FALSE)
      WritePrivateProfileString("Remote", "UserName", g_szUserName, "remote.ini");

   if(MsgBox("Do you want to specify password? If you specify password here, it will not be "
             "encripted and everyone will be able to see it", MB_YESNO) == IDYES)
   {
      if(Gte_GetStringDlg("Password", "&Password:", g_szPassword, sizeof(g_szPassword)) != FALSE)

//         return 
            WritePrivateProfileString("Remote", "Password",  g_szPassword, "remote.ini");
   }
...
void OpenTOC(void)
{
   ...
   if(hDoc == NULL) {
      MsgBox("Document must be open in editor, to find TOC file for it !", MB_OK);
      return 1;
   }
...
//###################################################################
{
   ...
   szFileName[0] = 0;
   if(Gte_BrowseForFileDlg(szFileName, sizeof(szFileName),
                       "DirStr file (dirstr.txt)|dirstr.txt|Txt files (*.txt)|*.txt|",
                       "Select file with directory structure to be created", FALSE, FALSE, NULL))
   {
   ...
   strcpy (szMessage,"Are youre shure you want to create directory structure based on this file, starting from ");
...
//###################################################################
{
      ...
      GetPrivateProfileString( "doc18-8-c"  ,"macro key 1 line 2", "", g_szDebug, FILENAME_MAX, "c:\\plwin\\plwin.ini");
      MsgBox(g_szDebug, MB_OK);

      WritePrivateProfileString("doc18-8-c", "macro key 1 line 2", "SEND ahaha", "c:\\plwin\\plwin.ini");
...
if(n4glVer == 0)
    ...
    else
    {
	  switch(n4glVer)
      {
         case 1: strcpy(g_szDebug,"Type 1 is local 4GL for DOS"); MsgBox(g_szDebug, MB_OK);
		            return;
         case 2: strcpy(g_szDebug,"Type 2 is local 4GL for Windows"); MsgBox(g_szDebug, MB_OK);
    	            return;
		 case 3: strcpy(g_szDebug,"Type 3 is remote Informix 4GL");MsgBox(g_szDebug, MB_OK);
		            return;
		 case 4: strcpy(g_szDebug,"Type 4 is remote 4Js/Informix D4GL GUI");MsgBox(g_szDebug, MB_OK);
		            return;
		 case 5: strcpy(g_szDebug,"Type 5 is remote 4Js/Informix D4GL CHUI");MsgBox(g_szDebug, MB_OK);
		            return;
       }
...
//###################################################################
{
   ...
   int  nIdx;
    ...
    else
    {
	  switch(n4glVer)
      {
         case 1: strcat(g_szDebug,"local 4GL for DOS)");
		            break;
         case 2: strcat(g_szDebug,"local 4GL for Windows)");
    	            break;
		 case 3: strcat(g_szDebug,"remote Informix 4GL)");
		            break;
		 case 4: strcat(g_szDebug,"remote 4Js/Informix D4GL GUI)");
		            break;
	  	 case 5: strcat(g_szDebug,"remote 4Js/Informix D4GL CHUI)");
		            break;
       }
    }
	  ...
	  switch(nIdx)
      {
         case 1: // keep current
		            return;
         case 2: WritePrivateProfileString("Remote", "4GLVersion", "1", "remote.ini");
					return;
		 case 3: WritePrivateProfileString("Remote", "4GLVersion", "2", "remote.ini");
		            return;
		 case 4: WritePrivateProfileString("Remote", "4GLVersion", "3", "remote.ini");
		            return;
		 case 5: WritePrivateProfileString("Remote", "4GLVersion", "4", "remote.ini");
		            return;
		 case 6: WritePrivateProfileString("Remote", "4GLVersion", "5", "remote.ini");
		            return;
