...
*/
          ...
          if(Gte_Find(hDoc, "#orig_location:", 1, 1, FALSE, FALSE, TRUE, TRUE) == 0)
		  {
			  ...
			  Gte_GetLine(hDoc, lStartLine, szSaveOrigLocation, sizeof(szOrigLocation));
		  ...
		  {
              Gte_GetCursorPos(hDoc, NULL, &lStartLine);
			  Gte_GetLine(hDoc, lStartLine, szSaveOrigFilename, sizeof(szOrigFilename));
            ...
            {
  			     Gte_UnlockUpdate(hDoc);
				 MsgBox("Cannot find #orig_filename: string in active doccument", MB_OK);
                 return;
          ...
          {
              Gte_GetCursorPos(hDoc, NULL, &lStartLine);
			  Gte_GetLine(hDoc, lStartLine, szSaveOrigHostname, sizeof(szOrigHostname));
          ...
          {
			     Gte_UnlockUpdate(hDoc);
				 MsgBox("Cannot find #orig_hostname: string in active doccument", MB_OK);
				 return;
          }
          ...
          {
              Gte_GetCursorPos(hDoc, NULL, &lStartLine);
			  Gte_GetLine(hDoc, lStartLine, szSaveOrigUsername, sizeof(szOrigUsername));
          ...
          {
  			     Gte_UnlockUpdate(hDoc);
			     MsgBox("Cannot find #orig_username: string in active doccument", MB_OK);
                 return;
          }
...
//    example: rcp -a \temp\PB1.4gl chaix.afalout:/apps/max_4.00/ap/PB1.4gl
   ...
   Gte_ExecuteTool(szExec, "c:\\temp\\rcp.log", "Returning file, please wait...", szExec);
