...
//int rval;
  ...
  suspend_flush (1);
  A4GL_trim (fname);
  send_to_ui ("      {\"type\":\"OPENWINDOWWITHFORM\",\"WINDOW\":\"%s\",\"X\":%d,\"Y\":%d,\"ATTRIBUTE\":%d,\"SOURCE\":\"%s\"", name, x, y, attrib, fname);
      send_to_ui
	(",\"TEXT\":\"%s\",\"STYLE\":\"%s\",\"ERROR_LINE\":%d,\"PROMPT_LINE\":%d,\"MENU_LINE\":%d,\"BORDER\":%d,\"COMMENT_LINE\":%d,\"MESSAGE_LINE\":%d,\"FORM_LINE\":%d,\"FORM\":",
...
UILIB_A4GL_open_form (char *name)
{
  ...
  send_to_ui ("      {\"type\":\"OPENFORM\",\"FORMNAME\":\"%s\",\"SOURCE\":\"%s\",\"Form\":", name, buff);
// Can we find a pregenerated XML form ? 
//
  if (!A4GL_JSON_opening_form (buff, name,1))
    {
...
{
  send_to_ui ("      {\"type\":\"CLOSEFORM\", \"FORMNAME\":\"%s\"},", name);
}
...
{
  int rval = 0;
  send_to_ui ("      {\"type\":\"DISPLAYFORM\", \"FORMNAME\":\"%s\",\"ATTRIBUTE\":%d},", name, attr);
  return rval;
}
...
{
  send_to_ui ("      {\"type\":\"CURRENTWINDOW\", \"WINDOW\":\"%s\"},", win_name);
  ...
  if (n == 4)
    {
      ...
      x3 = A4GL_pop_int ();
      x4 = A4GL_pop_int ();
      send_to_ui ("      {\"type\":\"DRAWBOX\", \"arg1\":%d,\"arg2\":%d,\"arg3\":%d,\"arg4\":%d },", x1, x2, x3, x4);
    }
  if (n == 5)
    {
      x1 = A4GL_pop_int ();
      ...
      x4 = A4GL_pop_int ();
      x5 = A4GL_pop_int ();
      send_to_ui ("      {\"type\":\"DRAWBOX\", \"arg1\":%d,\"arg2\":%d,\"arg3\":%d,\"arg4\":%d,\"arg5\":%d},", x1, x2, x3, x4, x5);
    }
...
UILIB_A4GL_req_field_input (void *sv, char type, va_list * ap)
{
  ...
  if (type == '+')
    {
      send_to_ui ("      {\"type\":\"NEXTFIELD\", \"CONTEXT\":%d,\"FIELD\":\"NEXT\"},", context);
      return 1;
    }
  if (type == '-')
    {
      send_to_ui ("      {\"type\":\"NEXTFIELD\", \"CONTEXT\":%d,\"FIELD\":\"PREVIOUS\"},", context);
      return 1;
    }
  if (type == '0')
    {
      send_to_ui ("      {\"type\":\"NEXTFIELD\", \"CONTEXT\":%d,\"FIELD\":\"CURRENT\"},", context);
      return 1;
    }
  ...
  if (list.field_name_list[0].fpos != 0 && list.field_name_list[0].fpos != 1)
    {
      send_to_ui ("      {\"type\":\"NEXTFIELD\", \"CONTEXT\":%d,\"FIELD\":\"%s\"},", context, get_field_with_subscript_as_string(list.field_name_list[0].fname, list.field_name_list[0].fpos));
    }
  else
    {
      send_to_ui ("      {\"type\":\"NEXTFIELD\",\"CONTEXT\":%d,\"FIELD\":\"%s\"},", context, list.field_name_list[0].fname);
    }
...
{
  send_to_ui ("      {\"type\":\"OPTIONS\",\"option\":\"%c\",\"value\":%d},", type, keyval);
}
...
UILIB_A4GL_gen_field_chars_ap (void *field_list, void *formdets, va_list * ap,int replace_0)
{
  ...
  while ((argp = va_arg (*ap, char *)))
	...
	{
	if (i==0) i=replace_0;
	  SPRINTF (smbuff, "            { \"Name\":\"%s\"}\n", get_field_with_subscript_as_string(argp, i));
	}
  ...
  if (x == -1 && y == -1)
	...
	if (A4GL_isyes(acl_getenv("DISPLAYLOCAL"))) {
		printf("%s\n",s);
      		...
      		send_to_ui ("      { \"type\":\"DISPLAY\",\"Data\":\"%s\"},", uilib_json_escape (char_encode(s)));
	}
    }
  else
    {
      send_to_ui ("      {\"type\":\"DISPLAYAT\", \"X\":%d,\"Y\":%d,\"ATTRIBUTE\":%d,\"Data\":\"%s\"},", x, y, a, uilib_json_escape (char_encode(s)));
    }
}
...
int cnt;
  ...
  send_to_ui_no_nl ("      {\"type\":\"PRINTFILE\",\"NAME\":\"%s\",\"PAGELENGTH\":%d,\"MODE\":\"%s\",\"Data\"=[", filename, pageLength,mode);
...
UILIB_A4GL_direct_to_ui (char *what, char *string)
{
  ...
  if (strcmp (what, "SET") == 0)
	...
	free(val);
      ...
      send_to_ui ("      {\"type\":\"UIOPTION\",\"NAME\":\"%s\",\"VALUE\":\"%s\"},", buff1,buff2);
      return;
	}
  ...
  if (strcmp (what, "dialog_setcursor") == 0)
    {
      ...
      int cpos;
	cpos=A4GL_pop_long();
      send_to_ui ("      {\"type\":\"SETCURSOR\",\"POSITION\":%d},",cpos);
      return;
    }
  ...
  if (def_stderr)
    {
      ...
      pipe_clear();
      send_to_ui ("      {\"type\":\"PROGRAMSTOP\", \"EXITCODE\":%d,\"ID\":%d,\"Data\":[", exitstatus, get_ui_id ('r'));
      if (def_stderr)
	{
    ...
    {
      pipe_clear();
      send_to_ui ("      {\"type\":\"PROGRAMSTOP\", \"EXITCODE\":%d,\"ID\":%d},", exitstatus, get_ui_id ('r'));
      unlink (stderr_fname);
    }
...
UILIB_A4GL_movewin (char *winname, int absol)
{
  ...
  send_to_ui ("      {\"type\":\"MOVEWINDOW\",\"WINDOW\":\"%s\"/,\"x\":%d,\"y\":%d,\"ABSOLUTE\":%d >", winname, x, y, absol);
...
{
  send_to_ui ("      {\"type\":\"HIDEWINDOW\",\"WINDOW\":\"%s\"},", winname);
...
{
  send_to_ui ("      {\"type\":\"SHOWWINDOW\",\"WINDOW\":\"%s\"},", winname);
...
{
  send_to_ui ("      {\"type\":\"CLEARWINDOW\",\"WINDOW\":\"%s\"},", winname);
}
...
UILIB_A4GL_remove_window (char *win_name)
{
  ...
  send_to_ui ("      {\"type\":\"CLOSEWINDOW\",\"WINDOW\":\"%s\"},", win_name);
  ...
  if (d->srec == 0)
    {
      ...
      SPRINTF (buff, "\"Fields\":[ { \"Name\":\"%s.*\" } ]", srecname);
...
{
  send_to_ui ("      {\"type\":\"CLEARFORM\", \"TODEFAULTS\":%d},", to_defaults);
}
		      ...
		      int comment_line, int message_line, int attrib, char *style, char *text)
{
  ...
  send_to_ui
    ("      {\"type\":\"CREATEWINDOW\", \"NAME\":\"%s\",\"X\":%d,\"Y\":%d,\"W\":%d,\"H\":%d,\"FORM_LINE\":%d,\"ERROR_LINE\":%d,\"PROMPT_LINE\":%d,\"MENU_LINE\":%d,\"COMMENT_LINE\":%d,\"MESSAGE_LINE\":%d,\"BORDER\":%d,\"ATTRIBUTE\":%d,\"STYLE\":\"%s\",\"TEXT\":\"%s\" },",
...
UILIB_A4GL_add_menu_timeout (void *menu, char timeout_type, int timeoutlen, int cmd_no_on_timeout, long *timeoutvar)
{
  ...
  mn_id++;
  send_to_ui
    ("      {\"CONTEXT\":%d,\"ID\":%d,\"CMD_NO\":%d,\"TYPE\":\"%c\",\"TIMEOUT\":%d },",
...
UILIB_A4GL_add_menu_action (void *menu, char *action, int cmd_no_on_timeout)
{
  ...
  mn_id++;
  send_to_ui
    ("         {\"CONTEXT\":%d,\"ID\":%d,\"ACTION\":\"%s\",\"CMD_NO_TIMEOUT\":%d },", context, mn_id, action, cmd_no_on_timeout);
}
...
UILIB_A4GL_read_metrics (void *formdetsv)
{
  ...
  if (generate_xml_forms) {
		...
		char buff[200];
		tmpnam(buff);
		fprintf(stderr,"Using XML form... %s", buff);
		//strcpy(buff,"tmp.xml");
		f=fopen(buff,"w");
		if (f) {
...
UILIB_A4GL_clr_fields_ap (int to_defaults, va_list * ap)
{
  ...
  send_to_ui ("      {\"type\":\"CLEAR\",\"todefault\":%d, %s },", to_defaults, field_list);
}
...
UILIB_aclfgl_a4gl_set_page (int n)
{
  int b;
  b = A4GL_pop_int ();
  send_to_ui ("      {\"type\":\"SETPAGE\",\"Page\":%d},", b);
  return 0;
}
...
UILIB_A4GL_acli_scroll_ap (int n, va_list * ap)
{
  ...
  arr = va_arg (*ap, char *);
  send_to_ui ("      {\"type\":\"SCROLL\",\"BY\":%d,\"ARRAY\":\"%s\"},", n, arr);
}
...
UILIB_aclfgl_fgl_set_arrline (int nparam)
{
  int b;
  b = A4GL_pop_int ();
  send_to_ui ("      {\"type\":\"SETARRLINE\", \"LINE\":%d},", b);
  return 0;
}
...
UILIB_aclfgl_fgl_set_scrline (int nparam)
{
  int b;
  b = A4GL_pop_int ();
  send_to_ui ("      {\"type\":\"SETSCRLINE\", \"LINE\":%d},", b);
  return 0;
}
...
void cleanup(void) {
      	pipe_clear();
      	send_to_ui ("      {\"type\":\"PROGRAMSTOP\",\"ID\":%d},",  get_ui_id ('r'));
	 flush_ui ();
...
if (n==0)  {
	send_to_ui("      {\"type\":\"CANCEL_INSERT\"},");
}	else {
	send_to_ui("      {\"type\":\"CANCEL_DELETE\"},");
}
}
