...
# +----------------------------------------------------------------------+
#
# $Id: curslib.c,v 1.133 2006-11-17 08:11:42 mikeaubury Exp $
#*/
...
A4GL_flatten_menu (ACL_Menu * menu)
{
  ...
  px = menu->gw_x;
  py = menu->gw_y;
  A4GL_debug ("px=%d py=%d menu->gw_b=%d menu->y=%d", px, py, menu->gw_b,
	      menu->y);
  if (menu->gw_b)
    {
      px -= 1;
  ...
  for (y = 0; y <= 1; y++)
      ...
      for (x = 0; x < menu->w; x++)
	{
	  ...
	  A4GL_debug ("%d - px+x=%d,py+y=%d %04x %c", x, x + px, y + py, a,
		      a & 0xff);
