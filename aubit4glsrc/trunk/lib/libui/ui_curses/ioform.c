...
void A4GL_set_field_pop_attr (FIELD * field, int attr,int cmd_type)
{
  ...
  if (attr != 0) A4GL_set_field_attr_with_attr (field, attr,cmd_type);
 ...
 */
int
A4GL_fgl_getfldbuf_ap (void *inp,va_list *ap)
{
...
A4GL_debug("fgl_fieldtouched");
  ...
  if (c >= 0)
	...
	if (fprop->flags&2)
	    {
	      A4GL_debug ("fieldtouched Field status is set for %p - %d", field_list[a],fprop->flags);
	      free (field_list);
	      return 1;
	    }
	}
