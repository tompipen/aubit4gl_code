...
GtkWidget *iarr_current_field=0;
 ...
 */
static void
debug_print_field_list (GtkWidget ** fld_list)
{
  int a;
...
//#define ILOOP_ACTION_MOVEGtkWidget
...
gen_field_list_vals (void *a, void *b, int nv, ...)
{
  ...
  va_start (va, nv);
  ptr = &va;
  return A4GL_gen_field_list_gtk (a, b, nv, &va);
}
...
gen_srec_field_list (char *s, struct s_form_dets *form, int a, int d)
{
  ...
  fld_list = (GtkWidget ***) calloc (d, sizeof (GtkWidget **));
 ...
 UILIB_A4GL_inp_arr (void *vinpa, int defs, char *srecname, int attrib, int init)
{
  ...
  if (inpa->srec == 0)
    {
      A4GL_debug ("inpaarr1");
      inpa->srec = A4GL_get_srec_gtk (srecname);
...
//  GtkWidget *firstfield = 0;
  ...
  field_list=malloc(sizeof(GtkWidget *)*((sio->srec->attribs.attribs_len+1) * sio->scr_dim)+1);
c=0;


  for (a=0;a<sio->scr_dim;a++) {
	for (b=0;b<sio->srec->attribs.attribs_len;b++) {
	field_list[c++]=sio->field_list[a][b];
	}
  }
  field_list[c]=0;
...
void
A4GL_mja_set_current_field (FORM * form, GtkWidget * field)
{
  int a;
...
*/
static void
A4GL_add_to_control_stack (struct s_inp_arr *sio, int op, GtkWidget * f,
...
// Sanity check the movements....
  ...
  if (arr_line != arr->arr_line)
	...
	arr->curr_line_is_new=0;
	  ...
	  A4GL_debug ("Adding AFTER GtkWidget..");
  ...
  else
    {
      ...
      A4GL_debug ("Adding AFTER GtkWidget ? %p", last_field);
...
process_control_stack (struct s_inp_arr *arr)
{
  ...
  a = arr->fcntrl_cnt - 1;
A4GL_debug("Process_control_stack : %d",arr->fcntrl[a].op);
...
*/
  ...
  if (arr->fcntrl[a].op == FORMCONTROL_BEFORE_FIELD)
      ...
      if (arr->fcntrl[a].state == 99)
	{
	  ...
	  //if (field_opts(arr->currentfield)&O_AUTOSKIP) {
	...
	printf("arr->scr_line=%d arr->curr_atrrib=%d arr->currentfield=%p",arr->scr_line, arr->curr_attrib,arr->currentfield);
...
*/
  ...
  if (arr->fcntrl[a].op == FORMCONTROL_AFTER_FIELD)
      ...
      if (ffc_rval != -4)
	{
	  ...
	  fprop = (struct struct_scr_field *) (field_userptr (arr->currentfield));
	  attr = A4GL_determine_attribute (FGL_CMD_INPUT, arr->display_attrib,
				      fprop, 0);
	  ...
	  if (attr != 0) A4GL_set_field_attr_with_attr (arr->currentfield, attr, FGL_CMD_INPUT);
                ...
                if ((fprop->datatype & DTYPE_MASK) != DTYPE_CHAR)
                  {
                    ...
                    A4GL_modify_size (&buff[4],A4GL_get_field_width(form->currentfield));
                                      //form->fileform->metrics.
                                      //metrics_val[A4GL_get_metric_no_curr
                                                  //( form->currentfield)].w);
...
/* This lot should be moved somewhere more appropriate... */
...
int A4GL_get_field_width(GtkWidget *w) {
  struct struct_scr_field *attr;
  int metric;
  struct struct_form *form;
  int width;
  form = A4GL_get_curr_form_xdr_form (1);

  attr = (struct struct_scr_field *)gtk_object_get_data (GTK_OBJECT (w), "Attribute");
  metric=A4GL_get_metric_no_curr(w);
  if (metric<0) {
		A4GL_debug("error getting metric...");
		return 0;
  }
  width=form->metrics.metrics_val[metric].w;
  A4GL_debug("Field width of %p = %d",w,width);
	return width;
...
void
A4GL_display_field_contents (GtkWidget * field, int d1, int s1, char *ptr1)
{
  int field_width;
  int has_format;
  ...
  A4GL_debug ("In display_field_contents ptr1=%p (*=%s)",ptr1,ptr1);
  ...
  if (has_format && !ignore_formatting)
    {
      ...
      if (strlen (A4GL_get_str_attribute (f, FA_S_FORMAT)) > field_width)
        {
	A4GL_debug("Format is wider that the field : %d %d", strlen (A4GL_get_str_attribute (f, FA_S_FORMAT)), field_width);
...
// I think these 2 can be removed...
          ...
          if (function)
            {
              ...
              A4GL_debug ("Returns %s , given %s (width %d)\n", ptr, ptr1,field_width);
            }
          else
            {
              ptr = 0;
  ...
  A4GL_pop_char (ff, field_width);
  A4GL_debug ("set_field_contents : '%s' (%d)", ff,field_width);
...
int
A4GL_get_metric_no_curr ( GtkWidget * f)
{
  int a;
  struct struct_form *form;
  form = A4GL_get_curr_form_xdr_form (1);

  A4GL_debug ("In curr metric %d metrics",form->metrics.metrics_len);

  for (a = 0; a < form->metrics.metrics_len; a++)
    {
      if (f == (GtkWidget*) form->metrics.metrics_val[a].field)
        {
          A4GL_debug ("Returning %d\n", a);
          return a;
        }
...
A4GL_comments (struct struct_scr_field *fprop)
{
  ...
  if (cline>A4GL_get_curr_height_gtk())  {
                cline=A4GL_get_curr_height_gtk();
  }
      ...
      if (A4GL_get_curr_border_gtk ())
        {
          ...
          while (l>A4GL_get_curr_height_gtk()) l--;
          return l ;
        }
      else
        {
...
/* -2 = last -1 */
  ...
  if (A4GL_get_curr_border_gtk ())
  ...
  else
    {
          l=0-l;
          l--;
          A4GL_debug ("Decoded (4) line %d to %d ", l, A4GL_get_curr_height_gtk ());
          return A4GL_get_curr_height_gtk () - l +1;
    }
...
int k=0;
	...
	if (action==1)  {
        ...
        if (action==2) {
		iarr_current_field= A4GL_get_which_field ();
		k=0;
		A4GL_clear_something ();
	}
	return k;
...
A4GL_get_curr_form_dets(int n)
{
	...
	w=A4GL_get_curr_form(n);

	A4GL_debug("Got curr_form as %d",w);
	if (w) {
        	fd1 = gtk_object_get_data (GTK_OBJECT (w), "currform");
		A4GL_debug("Got the form - look for the xdr_form in it...");
		if (fd1) {
			form=  gtk_object_get_data (GTK_OBJECT (fd1), "xdr_form");
			A4GL_debug("form = %p\n",form);
		}
	}
	return form;
