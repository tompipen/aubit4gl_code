...
static char _functionName[] = "splat_with_image";
   ...
   pop_var2(&w,2,0);
   /* End command */
   {debug("Line 613 assist.4gl:");}
   if (aclfgli_get_err_flg()&&(sqlca.sqlcode !=0 || status !=0 || 0)) {
   /* SQLERROR */
      if (sqlca.sqlcode<0&&status==sqlca.sqlcode) chk_err(613,_module_name);
   /* ERROR */
      if (status<0) chk_err(613,_module_name);
   }
   aclfgli_clr_err_flg();
   push_char("filename='");
   ...
   /* End command */
   {debug("Line 614 assist.4gl:");}
   if (aclfgli_get_err_flg()&&(sqlca.sqlcode !=0 || status !=0 || 0)) {
   /* SQLERROR */
      if (sqlca.sqlcode<0&&status==sqlca.sqlcode) chk_err(614,_module_name);
   /* ERROR */
      if (status<0) chk_err(614,_module_name);
   }
   aclfgli_clr_err_flg();
   CHK_UI
   {
