...
My curses.h is:
 ...
 $Id: lowlevel_tui.c,v 1.111 2008-02-08 13:53:35 mikeaubury Exp $ 
 #define NCURSES_VERSION_MAJOR 5
...
A4GL_LL_display_form (void *vf, int attrib, int curr_width, int curr_height,
		      ...
		      void *currwin, void *f_form, int maxline, int maxcol,char *name)
{
  int rows, cols;
  char buff[80];
  int a;
  ...
  if (rows - iscurrborder > curr_height + 1)
    {
	    A4GL_debug("%d %d %d",rows,iscurrborder,curr_height);
...
void LL_AddButtonToolbar(char *tag, char *buttonText,char *img, char *toolTip, int keyval,int AlwaysShow) {
	// does nothing - required by API
}
