...
# +----------------------------------------------------------------------+
#
# $Id: input_array.c,v 1.1 2004-01-12 14:00:03 mikeaubury Exp $
#*/
...
process_key_press (struct s_inp_arr *arr, int a)
{
  ...
  switch (act_as)
	...
	{
	  A4GL_newMovement (arr, 1, 1, arr->curr_attrib);
 ...
 */
int
UILIB_A4GL_inp_arr (void *vinpa, int defs, char *srecname, int attrib,
		    int init)
{
  void ***fld_list;
  int rval;
