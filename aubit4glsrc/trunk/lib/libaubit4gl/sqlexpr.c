...
get_select_list_item_i (struct s_select *select, struct s_select_list_item *p)
{
  ...
  switch (p->type)
    ...
    case E_SLI_IBIND:
      return strdup (p->u_data.expression);
    case E_SLI_VARIABLE:
      return strdup (p->u_data.expression);
    case E_SLI_DATETIME:
      return strdup (p->u_data.expression);
	...
	make_sql_string_and_free (get_select_list_item
				  (select, p->u_data.complex_expr.left),
				  strdup ("="), get_select_list_item (select,
								      p->
								      u_data.
								      complex_expr.
								      right),
...
/*
    ...
    case E_SLI_EXTEND:
      return make_sql_string_and_free (acl_strdup ("EXTEND ("),
				       get_select_list_item (select,
							     p->u_data.extend.
							     expr),
				       strdup (","), p->u_data.extend.from,
				       strdup (" TO "),
				       strdup (p->u_data.extend.to), kw_cb,
				       0);
	...
	return make_sql_string_and_free (strdup (p->u_data.column.colname),
					 strdup (buff), 0);
      }
		...
		if (select->select_list->list[a]->alias)
		  {
			...
			(select->select_list->list[a]->alias,
			 p->u_data.column.colname) == 0)
		      {
			return
			  make_sql_string_and_free (strdup
						    (p->u_data.column.
						     colname), 0);
		      }
		  }
	  ...
	  if (select->select_list->list[a]->type == E_SLI_BUILTIN_CONST_STAR)
	      ...
	      if (select->table_elements.ntables == 1)
		{
		  ...
		  select->select_list->list[a] =
		    new_select_list_item_col (strdup (tname), "*", 0);
		}
	      else
		{
		  expand_many = 1;
