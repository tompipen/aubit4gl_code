...
void *
A4GL_current_malloc_context (void)
{
  return malloc_context;
}

static int
has_malloc_context (void)
{
  if (malloc_context == 0)
    return 0;
  return 1;
...
void *
A4GL_get_malloc_context (void)
{
  return malloc_context;
}

void
A4GL_set_malloc_context (void *a)
{
  if (malloc_context)
    {
...
void
A4GL_free_malloc_context (void *s)
{
  A4GL_free_associated_mem (s);
}

void
A4GL_clr_malloc_context (void)
{
  malloc_context = 0;
}
...
void *
acl_malloc2_With_Context (long size)
{
  void *p;
  if (size > 2000000)
    {
  ...
  if (has_malloc_context ())
    {
      A4GL_set_associated_mem (A4GL_get_malloc_context (), p);
    }
  return p;
}
...
char *
acl_strdup_With_Context (void *a)
{
  char *p;
  p = strdup (a);
  A4GL_assertion (p == 0, "Unable to allocate memory");
...
void *
acl_realloc_With_Context (void *a, long b)
{
  void *p;
  if (has_malloc_context ())
    {
      A4GL_rm_associated_mem (A4GL_get_malloc_context (), a);
    }
  p = realloc (a, b);
  A4GL_assertion (p == 0, "Unable to allocate memory");
  if (has_malloc_context ())
...
void
A4GL_rm_associated_mem (void *orig, void *newbytes)
{
  char buff[256] = "";
  //int nelem;
 ...
 */
void *
A4GL_set_associated_mem (void *orig, void *newbytes)
{
  char buff[256] = "";
  int nelem;
  void *newblock = 0;
 ...
 */
void *
A4GL_alloc_associated_mem (void *orig, int nbytes)
{
  char buff[256] = "";
  int nelem;
  struct mem_extra *ptr = 0;
...
void
A4GL_free_associated_mem (void *orig)
{
  char buff[256];
