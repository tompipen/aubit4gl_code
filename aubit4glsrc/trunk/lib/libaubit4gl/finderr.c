...
static char *check_for_msgno(char *file, int msgno) {
char *lv_char;
long lv_msgline;
				A4GL_push_char(file);
...
static char *internal_get_errmsg_from_helpfile(char *path, int n) {
...
ptr=internal_get_errmsg_from_helpfile(buff,n); if (ptr) return ptr;
...
if (!A4GL_isno(acl_getenv("CHECKINFORMIXMSGS"))) {
	...
	if (acl_getenv_not_set_as_0("INFORMIXDIR")!=0) {
		...
		A4GL_debug("Looking in directory %s\n",buff);
		ptr=internal_get_errmsg_from_helpfile(buff,n); if (ptr) {
			char *p;
			p=strchr(ptr,'\n'); if (p) *p=0;
			p=strchr(ptr,'\r'); if (p) *p=0;
			return ptr;
		}
	}
}
