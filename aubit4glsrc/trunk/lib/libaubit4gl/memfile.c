...
*/
  ...
  for (a = 0; a < ncomments; a++)
    {
      A4GL_debug ("%d %d - %s\n", load_comments[a].lineno, load_comments[a].colno, load_comments[a].comment);
    }
#endif
}
...
int
A4GL_GetNumberOfComments (void)
{
  return ncomments;
}

int
A4GL_GetComment (int a, char **s, int *l, int *c, char *type)
{
  if (a < ncomments)
    {
...
char *
A4GL_has_comment (int n, int c)
{
  int a;
  for (a = 0; a < ncomments; a++)
    {
      if (load_comments[a].printed)
	continue;
...
static void
add_comment (int n, int c, char *s, char type)
{
  ncomments++;
