...
clr_bindings (void)
{
  ...
  input_bind->unexpanded_list.list.list_len = 0;
  input_bind->unexpanded_list.list.list_val = 0;
  ...
  output_bind->unexpanded_list.list.list_len = 0;
  output_bind->unexpanded_list.list.list_val = 0;
}
...
char *
get_sql_into_buff (struct expr_str_list *into)
{
...
void
print_report_table (char *repname, char type, int c, char *asc_desc, struct expr_str_list *funcbind,
		    struct expr_str_list *orderbind)
{
...
/******************************************************************************/
static void
print_use_session (expr_str * con)
{
  if (con == NULL)
    return;
...
void
print_exists_subquery (int i, struct s_expr_exists_sq *e)
{
  //int n;
  struct s_select *s;
  //expr_str_list l;
  ...
  if (i)
    printc ("A4GL_pushop(OP_EXISTS);");
  else
    printc ("A4GL_pushop(OP_NOTEXISTS);");
  //printc("free(_sql);");
  printc ("}");
}
...
void
print_in_subquery (int i, struct s_expr_in_sq *e)
{
  //int n;
  struct s_select *s;
  //expr_str_list l;
  ...
  if (i)
    printc ("A4GL_pushop(OP_IN_SELECT);");
  else
    printc ("A4GL_pushop(OP_NOTIN_SELECT);");
  //printc("free(_sql);");
  printc ("}");
}
...
/******************************************************************************/
int
print_close_sql_cmd (struct_close_sql_cmd * cmd_data, int already_in_command)
{
...
print_sql_transact_cmd (struct_sql_transact_cmd * cmd_data)
{
  ...
  switch (cmd_data->trans)
    {
    case -1:			// Begin
      printc ("A4GLSQL_commit_rollback(%d);", cmd_data->trans);
      break;
    case 1:			// Commit
      printc ("A4GLSQL_commit_rollback(%d);", cmd_data->trans);
      break;
...
// ---- 
  ...
  if (input_bind && input_bind->list.list_len)
    ...
    {
      // No variables used...
      printc ("A4GL_execute_implicit_sql(A4GL_prepare_select(NULL,0,0,0,\"%s\",_module_name,%d,%d,0),1,0,0);\n", ptr, line_for_cmd,
	      converted);
    }
...
//printf("sql=%s\n",sql);
  ...
  if (cmd_data->sql->sf && cmd_data->sql->sf->into_temp && strlen (cmd_data->sql->sf->into_temp))
    {
      printc ("A4GL_execute_implicit_sql(%s,1,0,0); /* 0 */\n\n", b2);
    }
  else
    {
      printc ("A4GL_execute_implicit_select(%s,1); /* 0 */\n\n", b2);
    }
...
/******************************************************************************/
static char *
generate_sid_string_for_declare (struct s_cur_def *declare_dets, int *forUpdate)
{
...
// ---- 
  ...
  if (cmd_data->inbind && cmd_data->inbind->list.list_len)
    using += 1;
...
// Collect any 'intos...'
  ...
  printc ("A4GL_add_prepare(\"%s\",(void *)A4GL_prepare_select(0,0,0,0,\"%s\",_module_name,%d,%d,0));", tmpbuff,
	  escape_quotes_and_remove_nl (buff), line_for_cmd, cmd_data->convert == 0 /* never converted */ );
...
*/
  ...
  if (input_bind->list.list_len == 0)
    return sorig;

  // Does it contain some numbered parameters  ? 
  if (strstr (sorig, A4GL_PARAM_START) == 0)
    return sorig;
  ...
  if (need_to_change)
      ...
      if (A4GL_isyes (acl_getenv ("ERRCHECKPARAM")))
	{
      ...
      new_input_bind->unexpanded_list.list.list_len = 0;
      new_input_bind->unexpanded_list.list.list_val = 0;
