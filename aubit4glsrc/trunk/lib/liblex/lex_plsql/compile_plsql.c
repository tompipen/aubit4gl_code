...
struct s_declares {
	int declare_type;
	struct_declare_cmd *c;
	int nvars;
	char **vars;
};

struct s_declares declares[2000];
...
static int  add_declare(int declare_now, struct_declare_cmd *c) {

	declares[ndeclares].declare_type=declare_now;
	declares[ndeclares].c=c;
	declares[ndeclares].nvars=0;
	declares[ndeclares].vars=0;
	ndeclares++;

	if (ndeclares>=2000) {
			printf("Too many declares\n");
			exit(2);
	}
	return ndeclares-1;
}
...
static void add_declare_var(int d,int p, char *s) {
	declares[d].nvars=p+1;
	declares[d].vars=realloc(declares[d].vars,sizeof(char *) *(declares[d].nvars+1));
	declares[d].vars[p]=strdup(s);
}

static int find_declare_pos(char *s) {
int a;
char *ptr;
for (a=0;a<ndeclares;a++) {
		ptr=get_ident(declares[a].c->cursorname);
		if (strcmp(s, ptr)==0) {
			return a;
		}
	}
...
add_vname (char *f, char *v, int dtype)
{
  ...
  sprintf (buff, "%-16s %s;", v, buff_dtype);
...
static char *
search_variables_declare (int declare_pos, struct s_select_list_item_list *l)
{
  int a;
int vcnt=0;
static char buff_vars[20000];
...
strcpy(buff_vars,"");
  ...
  for (a = 0; a < l->list.list_len; a++)
      ...
      if (l->list.list_val[a]->data.type == E_SLI_VARIABLE_USAGE)
	{
		...
		add_declare_var(declare_pos, vcnt, get_variable_usage(l->list.list_val[a]->data.s_select_list_item_data_u.var_usage));


		vu=usage_bottom_level( l->list.list_val[a]->data.s_select_list_item_data_u.var_usage);
			
                //printf("%s\n",get_variable_usage (l->list.list_val[a]->data.s_select_list_item_data_u.var_usage));
	  	//ptr=get_variable_dtype_from_variable_usage_expression(errbuff, l->list.list_val[a]->data.s_select_list_item_data_u.var_usage);
	  	sprintf(buff,"fgli%d %s", vcnt, get_spl_dtype(vu->datatype));

		if (vcnt) strcat(buff_vars,",");
		strcpy(buff_vars,buff);

		sprintf(buff,"fgli%d",vcnt);
	  	l->list.list_val[a]->data.type = E_SLI_VARIABLE;
	  	l->list.list_val[a]->data.s_select_list_item_data_u.expression = strdup(buff);
		vcnt++;
...
if (vcnt) {
	sprintf(buff_vars2,"(%s)", buff_vars);
	strcpy(buff_vars,buff_vars2);
}
...
static char* get_ident (struct expr_str *ptr)
{
  if (ptr->expr_type == ET_EXPR_IDENTIFIER)
    {
      return get_orig_from_clobber (ptr->expr_str_u.expr_string);
    }
  ...
  if (ptr->expr_type == ET_EXPR_VARIABLE_USAGE)
    {
      return  get_variable_usage (ptr->expr_str_u.expr_variable_usage);
    }
...
char *A4GL_strip_single_quotes(char *s) {
	if (strchr(s,'\'')) {
		printf("Need to fix string..\n");
	}
	return s;
}
...
local_get_expr_as_string (struct expr_str *ptr)
{
  ...
  switch (ptr->expr_type)
    ...
    case ET_EXPR_LITERAL_STRING:
      {
	char buff[60000];
	sprintf (buff, "\'%s\'", A4GL_strip_single_quotes (ptr->expr_str_u.expr_string));
	return acl_strdup (buff);
      }
      break;
      ...
      for (a = 0; a < l->list.list_len; a++)
	{
	  if (tolower (get_var_expr_scope (l->list.list_val[a])) == 'm')
		...
		need_daylight ();	//printc ("#");
	      ...
	      printc ("lv_tmpi_%x_%d:=", get_var_expr_dtype (l->list.list_val[a]), a);
...
print_Constant_1 (char *name, struct constant_data *c)
{
  switch (c->consttype)
    ...
    case CONST_TYPE_CHAR:
      printc ("%s CONSTANT %s", name, c->constant_data_u.data_c);
      break;
    case CONST_TYPE_IDENT:
      printc ("%s CONSTANT %s", name, c->constant_data_u.data_ident);
      break;
    case CONST_TYPE_INTEGER:
      printc ("%s CONSTANT %d", name, c->constant_data_u.data_i);
      break;
    case CONST_TYPE_FLOAT:
      printc ("%s CONSTANT %lf", name, c->constant_data_u.data_f);
      break;
...
LEXLIB_A4GL_write_generated_code (struct module_definition *m)
{
  ...
  A4GL_debug ("Dump 3");
  A4GL_setenv("A4GL_TARGETDIALECT","POSTGRES8",1);
  ...
  for (a = 0; a < func_cmds->cmds.cmds_len; a++)
	...
	if (r->cmd_data.type==E_CMD_DECLARE_CMD) {
		...
		if (r->cmd_data.command_data_u.declare_cmd.declare_dets->ident) {
			z=add_declare(0, &r->cmd_data.command_data_u.declare_cmd);
			sprintf(buff," cur_%s refcursor;\n", get_ident(r->cmd_data.command_data_u.declare_cmd.cursorname));
		} else {
			char *ptr;
			z=add_declare(1, &r->cmd_data.command_data_u.declare_cmd);
                	preprocess_sql_statement (r->cmd_data.command_data_u.declare_cmd.declare_dets->select);
                	ptr=search_variables_declare (z, &r->cmd_data.command_data_u.declare_cmd.declare_dets->select->list_of_items);
			sprintf(buff," cur_%s cursor %s for ", get_ident(r->cmd_data.command_data_u.declare_cmd.cursorname), ptr);
                	strcat(buff,get_select (r->cmd_data.command_data_u.declare_cmd.declare_dets->select, r->cmd_data.command_data_u.declare_cmd.declare_dets->forUpdate));
			strcat(buff,";\n");
		}
		strcat(mainbuff,buff);
	}
	if (r->cmd_data.type==E_CMD_PREPARE_CMD) {
		char buff[20000];
		sprintf(buff," prep_%s char(1024);\n",get_ident(r->cmd_data.command_data_u.prepare_cmd.stmtid));
		strcat(mainbuff,buff);
		sprintf(buff," exec_prep_%s char(1024);\n",get_ident(r->cmd_data.command_data_u.prepare_cmd.stmtid));
		strcat(mainbuff,buff);
...
void
print_open_cursor (expr_str * cursorname, expr_str_list * inputvals)
{
  int a;
  int z;
  struct_declare_cmd *d;
  z = find_declare_pos (get_ident (cursorname));

  if (inputvals)
    {
      if (inputvals->list.list_len == 0)
	{
	  inputvals = 0;
	}
    }
  ...
  if (declares[z].declare_type == 0)
    {
      char buff[10000];
	char pname[2000];

        printc("exec_prep_%s:=prep_%s;",   get_ident (declares[z].c->declare_dets->ident), get_ident (declares[z].c->declare_dets->ident));

	sprintf(pname, "exec_prep_%s", get_ident (declares[z].c->declare_dets->ident));
 	
      if (inputvals)
	{
	  for (a = 0; a < inputvals->list.list_len; a++)
	    {
		set_nonewlines();
		printc("%s:=replace(%s,'$%d',quote_literal(",pname,pname,a+1);
	        print_varbind (inputvals->list.list_val[a], 'r', a);
		printc("));");
		clr_nonewlines();
	    }
      ...
      else
	{
	  ...
	  if (declares[z].vars)
	    {
	      int b;
	      for (b = 0; b < declares[z].nvars; b++)
		{
		printc("%s:=replace(%s,'$%d',quote_literal(%s));",pname,pname,a+1, declares[z].vars[b]);
		}
	    }
	}
  ...
  if (inputvals == 0)
      ...
      else
	{
      ...
      if (d == 0)
	{
	  printf ("WARNING: Unable to locate cursor : %s\n", get_ident (cursorname));
	}
      else
	{
	  char buff[10000];
...
dump_cmd (struct command *r, struct command *parent)
{
  ...
  switch (r->cmd_data.type)
	...
	*/
	...
	case EBC_FOREACH:
	  printc ("exit foreach_%d;",  r->cmd_data.command_data_u.ext_cmd.block_id);
	  break;
	...
	case EBC_INPUT:
	  ...
	  break;
	case EBC_FOREACH:
	  printc ("continue foreach_%d;",  r->cmd_data.command_data_u.ext_cmd.block_id);
	  break;
    ...
    case E_CMD_FOREACH_CMD:
      ...
      loop_descent_level++;
      need_daylight ();		//printc ("#");
      print_open_cursor(r->cmd_data.command_data_u.foreach_cmd.cursorname, r->cmd_data.command_data_u.foreach_cmd.inputvals);

      printc ("<<foreach_%d>> WHILE TRUE LOOP", r->cmd_data.command_data_u.foreach_cmd.block_id);
	//printc("label continue_foreach_%d:",r->cmd_data.command_data_u.foreach_cmd.block_id);
      tmp_ccnt++;
      set_nonewlines ();
      printc("FETCH cur_");
	...
	printc("IF NOT FOUND THEN");
	set_nonewlines();
	printc("   CLOSE cur_");
	print_ident(r->cmd_data.command_data_u.foreach_cmd.cursorname);
	printc(";");
	clr_nonewlines();
	printc("   EXIT foreach_%d;", r->cmd_data.command_data_u.foreach_cmd.block_id);
	printc("END IF;");
      ...
      tmp_ccnt--;
      need_daylight ();
	//printc("label exit_foreach_%d:",r->cmd_data.command_data_u.foreach_cmd.block_id);
      printc ("END LOOP;");
      need_daylight ();
      //printc ("#");
      break;
	...
	if (r->cmd_data.command_data_u.init_cmd.tablist && 0 )
	  {
	    printc (" LIKE ");
	...
	print_open_cursor(r->cmd_data.command_data_u.open_cursor_cmd.cursorname, r->cmd_data.command_data_u.open_cursor_cmd.using_bind);
/*
      set_nonewlines ();
      printc ("OPEN ");
...
*/
	...
	printc("prep_%s:=convert_to_pgsql_placeholders(",get_ident(r->cmd_data.command_data_u.prepare_cmd.stmtid));
	...
	ptr= A4GLSQLCV_check_sql (r->cmd_data.command_data_u.sql_cmd.sql, &converted);
	if (strlen(ptr)) {
      	printc ("%s;", ptr);
	}
      	clr_nonewlines ();
