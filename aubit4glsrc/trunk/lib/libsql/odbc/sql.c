...
void *A4GL_bind_decimal (void *parent, void *ptr_to_decimal);
        ...
        SQLINTEGER FAR * pcbValue)
{
    ...
    A4GL_dbg ("Setting parameter %d to type %d (%d) %d (%d)", ipar, fSqlType, SQL_C_BINARY, fCType, SQL_PARAM_INPUT);
 ...
 */
void *
A4GLSQLLIB_A4GLSQL_declare_cursor_internal (int upd_hold, void *vsid,
...
int allocated_nsid=0;
	...
	if (upd_hold&2) {
	    ...
	    //{
		//rc = SQLSetStmtAttr ((SQLHSTMT) nsid->hstmt, SQL_ATTR_CURSOR_HOLD , (SQLPOINTER) SQL_CURSOR_HOLD_ON , 0);
	    //} else {
	        //rc = SQLSetStmtOption ((SQLHSTMT) nsid->hstmt, SQL_ATTR_CURSOR_HOLD , SQL_CURSOR_HOLD_ON);
	    //}
#else
	        //rc = SQLSetStmtOption ((SQLHSTMT) nsid->hstmt, SQL_ATTR_CURSOR_HOLD , SQL_CURSOR_HOLD_ON);
#endif
 ...
 * @return 1 = BAD, 0= OK
 */
int A4GLSQLLIB_A4GLSQL_open_cursor_internal (char *s, int ni, void *ibind)
{
    struct s_cid *cid;
    char *curs;
    SQLINTEGER rowcount;
 ...
 */
void
A4GLSQLLIB_A4GLSQL_free_cursor_internal (char *cname)
{
    struct s_cid *ptr;
...
A4GL_obind_column (int pos, struct BINDING *bind, HSTMT hstmt)
{
  ...
  rc=SQLNumResultCols(hstmt,&nrc);
  if (rc<0) {
		nrc=-1;
  } else {
		// Binding more outputs than we have in the select..
		if (pos>nrc) {
			return 1;
		}
  }
  ...
  if (bind->dtype == DTYPE_DATE)
    {
      bind->libptr = A4GL_bind_date (hstmt, (long *) bind->ptr);
      ptr_to_use = bind->libptr;
    }

  if (bind->dtype == DTYPE_DTIME)
    {
      bind->libptr = A4GL_bind_datetime (hstmt, (void *) bind->ptr);
      ptr_to_use = bind->libptr;
    }

  if (bind->dtype == DTYPE_INTERVAL)
    {
      bind->libptr = A4GL_bind_interval (hstmt, (void *) bind->ptr);
      ptr_to_use = bind->libptr;
    }

  if (bind->dtype == DTYPE_DECIMAL || bind->dtype == DTYPE_MONEY)
    {
      bind->libptr = A4GL_bind_decimal (hstmt, (void *) bind->ptr);
      ptr_to_use = bind->libptr;
    }
  ...
  A4GL_assertion (conv_4gl_to_c[bind->dtype] ==-1,
  ...
  if (!chk_rc(rc, hstmt, "SQLBindCol")) {
	A4GL_debug("Failed to bind column.... *************************************************************");
      return False;
  }
...
A4GL_ibind_column (int pos, struct BINDING *bind, HSTMT hstmt)
{
    ...
    if (bind->dtype == DTYPE_DATE)
    {
        ...
        ptr = bind->ptr;
        p = (ACLDATE *) A4GL_bind_date (hstmt,(long *) ptr);
    ...
    if (bind->dtype == DTYPE_DTIME)
    {
        ...
        p = (ACLDTIME *) A4GL_bind_datetime (hstmt, ptr);
        if (dtime_as_char)
        {
            char buff[50];
    ...
    A4GL_dbg ("%d DTYPE %d DTYPE %d SIZE %d", bind->dtype, conv_4gl_to_c[bind->dtype],
...
static int sql_get_next_column_info(struct sql_col_info_data *ci)
{
    ...
    fgldtype = conv_sqldtype(ci->dtype, ci->prec) & DTYPE_MASK;
                                        ...
                                        int *prec, int *raw_dtype)
{
    ...
    if (colname)
        *colname = colname_buf;
    if (dtype)
        *dtype = conv_sqldtype(cidata.dtype, cidata.prec) & DTYPE_MASK;
    if (size)
        *size = cidata.size;
    if (prec)
        *prec = cidata.prec;
 ...
 */
void *
A4GL_bind_date (void *parent, long *ptr_to_date_var)
{
    ACLDATE *ptr;

    A4GL_trc ("Binding date for %p", ptr_to_date_var);
    ptr = A4GL_alloc_associated_mem (parent, sizeof (ACLDATE));
...
void *
A4GL_bind_datetime (void *parent, void *ptr_to_dtime_var)
{
    ACLDTIME *ptr;

    A4GL_trc ("Binding datetime for %p", ptr_to_dtime_var);
    ptr = A4GL_alloc_associated_mem (parent, sizeof (ACLDTIME));
...
void *
A4GL_bind_interval (void *parent, void *ptr_to_ival)
{
    ACLIVAL *ptr;

    A4GL_trc ("Binding interval for %p", ptr_to_ival);
    ptr = A4GL_alloc_associated_mem (parent, sizeof (ACLIVAL));
...
void *
A4GL_bind_decimal (void *parent, void *ptr_to_decimal)
{
    double *ptr;
    A4GL_trc ("Binding interval for %p", ptr_to_decimal);
    ptr = A4GL_alloc_associated_mem (parent, sizeof (double));
    return (void *) ptr;
}
                                ...
                                HSTMT hstmt)
{
    ...
    {
	switch (use_binding[bind_counter].dtype) {
		/* We need to check any types that use 'libptr'.. */
 		/* If libptr is not set - then we've got more values in our output that in our select.. */
		...
		case DTYPE_DECIMAL:
		case DTYPE_MONEY:
			if (use_binding[bind_counter].libptr==NULL) {
				continue;
			}
	}
        ...
        {
            A4GL_dbg ("Found string @ %d = '%s'", bind_counter, use_binding[bind_counter].ptr);
		...
		if (date1->uDate.date_ds.year==-1 || (date1->uDate.date_ds.year==0 && date1->uDate.date_ds.month==0 && date1->uDate.date_ds.day==0))  { // Null/no rows found..
                    A4GL_setnull (DTYPE_DATE, date1->ptr, 0);
		} else {
...
SQLINTEGER *StrLen_or_Ind);
   ...
   A4GL_set_a4gl_sqlca_errd(2,cnt);
...
aclfgli_clr_err_flg();
    A4GL_add_prepare ("p_get_val2", (void *) A4GL_prepare_select( 0, 0, 0, 0, buff,"__internal_sql_1", 0,0,0));
    if (a4gl_sqlca.sqlcode != 0)
        return NULL;
    A4GL_declare_cursor (0 + 0, A4GL_find_prepare ("p_get_val2"), 0, "c_get_val2");
    if (a4gl_sqlca.sqlcode != 0)
        return NULL;
    A4GL_open_cursor ("c_get_val2", 0, 0);
    if (a4gl_sqlca.sqlcode != 0)
        return NULL;
...
int bad;
   ...
   struct s_cid *cid;
    ...
    if (!A4GL_add_prepare ("p_get_val", (void *) A4GL_prepare_select(0, 0, 0, 0, buff,"__internal_sql_1", 0,0,0))) {
		return  (void *) -1;
    }
    ...
    cid=A4GL_declare_cursor (0 + 0, A4GL_find_prepare ("p_get_val"), 0, "c_get_val");

    if (a4gl_sqlca.sqlcode != 0) {
        return (void *) -1;
    }
    bad=A4GL_open_cursor ("c_get_val", 0, 0);

    if (a4gl_sqlca.sqlcode != 0 || bad) {
        return (void *) -1;
	}
...
//        nativeerr = -101;    //  word of comment why someone might need this?
    ...
    if (A4GL_set_status (nativeerr, 0))
    {
