...
# +----------------------------------------------------------------------+
#
# $Id: has_pdf.c,v 1.10 2003-05-12 14:24:23 mikeaubury Exp $
#*/
...
void pdf_rep_print (struct pdf_rep_structure *rep, int a, int s,
		    int right_margin);
...
int pdf_blob_print (struct pdf_rep_structure *p, struct fgl_int_loc *blob,
		    char *type, int cr);
...
pdf_size (double f, char c, struct pdf_rep_structure *p)
{
  ...
  if (f < 0)
    ...
    return pdf_metric ((int) f, c, p);	/* This is in lines or chars */
 ...
 */
int
pdf_blob_print (struct pdf_rep_structure *p, struct fgl_int_loc *blob,
		char *type, int cr)
{
  int n;
  double sx;
