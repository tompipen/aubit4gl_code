...
static int
local_isGenStackInfo (void)
{
  return current_module->genStackInfo == EB_TRUE;
}

int
local_is_system_variable (struct variable *v)
{
  return v->user_system == 'S';
}

static void
set_doing_a_report_call (void)
{
  if (is_in_report () == 2)
    {
...
char *
get_last_print_bind_dir_definition_g_rval (char dir)
{
...
//
char *
get_current_module_name_clean (void)
{
  char *ptr;
  static char buff[2000];
...
void
print_copy_status_not_sql (int l)
{
...
void
print_copy_status_with_sql (int l)
{
...
dump_cmd (struct command *r,int isAtModuleLevel)
{
  ...
  switch (current_entry->met_type)
    ...
    case E_MET_FUNCTION_DEFINITION:
      printc ("/* CMD : %s Line %d %s */\n", decode_cmd_type (r->cmd_data.type), r->lineno,
	      decode_module_entry_type (current_entry->met_type));
...
open_outfile (void)
{
  ...
  if (doing_esql ())
	...
	case E_DIALECT_INFORMIX:
	  if (strcmp (A4GL_get_esql_ext (), ".ec") != 0)
	    printf ("Unexpected extension for informix\n");
	  break;

	case E_DIALECT_POSTGRES:
	  if (strcmp (A4GL_get_esql_ext (), ".cpc") != 0)
	    printf ("Unexpected extension for postgres\n");
	  break;

	case E_DIALECT_SAPDB:	/* SAPDB pre-compiler also uses .cpc extension */
	  if (strcmp (A4GL_get_esql_ext (), ".cpc") != 0)
	    printf ("Unexpected extension for SAP\n");
	  break;

	case E_DIALECT_INGRES:
	  if (strcmp (A4GL_get_esql_ext (), ".sc") != 0)
	    printf ("Unexpected extension for INGRES\n");
	  break;

	case E_DIALECT_INFOFLEX:
	  if (strcmp (A4GL_get_esql_ext (), ".ec") != 0)
	    printf ("Unexpected extension for infoflex\n");
	  break;
	default:
 ...
 */
void
print_rep_ret (int report_cnt, int addit)
{
  if (addit)
    printc ("A4GL_%spop_report_section(&_rep,%d);", generate_ispdf (), rep_print_code++);
...
char *
print_arr_expr_fcall (void *optr)
{
  struct expr_str *ptr;
...
real_print_expr (struct expr_str *ptr)
{
  ...
  switch (ptr->expr_type)
    ...
    case ET_EXPR_CURRENT:
      printc ("A4GL_push_current(%d,%d);", ptr->expr_str_u.expr_current->from, ptr->expr_str_u.expr_current->to);
      break;
    ...
    case ET_EXPR_YEAR_FUNC:
      real_print_expr (ptr->expr_str_u.expr_expr);
      printc ("aclfgl_year(1);");
      break;

    case ET_EXPR_DATE_FUNC:
      real_print_expr (ptr->expr_str_u.expr_expr);
      printc ("aclfgl_date(1);");
      break;

    case ET_EXPR_MONTH_FUNC:
      real_print_expr (ptr->expr_str_u.expr_expr);
      printc ("aclfgl_month(1);");
      break;
      ...
      if (A4GL_doing_pcode ())
	{
	  if (ptr->expr_str_u.expr_infield->sio_id != -1)
	    {
	      printc ("A4GL_pushint_fgl_infield(&_sio_%d,_inp_io_type,%s,NULL,0);", (int) ptr->expr_str_u.expr_infield->sio_id,
		      local_field_name_list_as_char (ptr->expr_str_u.expr_infield->field_list));
	    }
	  else
	    {
	      printc ("A4GL_pushint_fgl_infield(NULL,0,%s,NULL,0);",
		      local_field_name_list_as_char (ptr->expr_str_u.expr_infield->field_list));
	    }
	}
      else
	{
	  if (ptr->expr_str_u.expr_infield->sio_id != -1)
	    {
	      printc ("A4GL_push_int(A4GL_fgl_infield(&_sio_%d,_inp_io_type,%s,NULL,0));",
		      (int) ptr->expr_str_u.expr_infield->sio_id,
		      local_field_name_list_as_char (ptr->expr_str_u.expr_infield->field_list));
	    }
	  else
	    {
	      printc ("A4GL_push_int(A4GL_fgl_infield(NULL,0,%s,NULL,0));",
		      local_field_name_list_as_char (ptr->expr_str_u.expr_infield->field_list));
	    }
	}
      break;
	...
	{
	  printc ("A4GL_push_int(A4GL_fgl_fieldtouched_current(&_sio_%d,_inp_io_type));",
		  (int) ptr->expr_str_u.expr_field_touched->sio_id);
	}
      else
	{
	  printc ("A4GL_push_int(A4GL_fgl_fieldtouched(&_sio_%d,_inp_io_type,%s,NULL,0));",
		  (int) ptr->expr_str_u.expr_field_touched->sio_id,
		  local_field_name_list_as_char (ptr->expr_str_u.expr_field_touched->field_list));
	}
      break;
    ...
    case ET_EXPR_FGL_SIZEOF:
      printc ("A4GL_push_long(sizeof(%s));",
	      generation_get_variable_usage_as_string (ptr->expr_str_u.expr_expr->expr_str_u.expr_variable_usage));
      break;
    ...
    case ET_EXPR_FGL_ADDRESSOF:
      printc ("A4GL_push_long((long)&(%s));",
	      generation_get_variable_usage_as_string (ptr->expr_str_u.expr_expr->expr_str_u.expr_variable_usage));
      break;

    case ET_EXPR_FGL_ISDYNARR_ALLOCATED:
      printc ("A4GL_isdynarr_allocated(&%s);",
	      generation_get_variable_usage_as_string (ptr->expr_str_u.expr_expr->expr_str_u.expr_variable_usage));
      break;

    case ET_EXPR_FGL_DYNARR_EXTENTSIZE:
      printc ("A4GL_dynarr_extent(&%s,%d);",
	      generation_get_variable_usage_as_string (ptr->expr_str_u.expr_dynarr_extent->var->expr_str_u.expr_variable_usage),
	...
	("if (_retvars != 1 ) {A4GL_set_status(-3001,0);A4GL_chk_err(%d,\"%s\");}",
	 ptr->expr_str_u.expr_get_fldbuf->line, ptr->expr_str_u.expr_get_fldbuf->module);
...
//int frm_attr;
  ...
  printc ("%d,%s,%s,%s,%s,%d,%s,%s,_attr",
	  iswindow, buffer[0], buffer[1], buffer[2], buffer[3], form_attrib->border == EB_TRUE, buffer[4], buffer[5]);
  free (buffer[0]);
  free (buffer[1]);
  free (buffer[2]);
  free (buffer[3]);
  ...
  A4GL_debug ("%d,%d,%d,%d,%d,%d,%d,%d,%s", form_attrib->iswindow,
	      ...
	      form_attrib->border, form_attrib->comment_line, form_attrib->message_line, frm_attr);
#endif
}
...
A4GL_rationalize_list_concat (struct expr_str_list *l)
{
  ...
  if ((l->list.list_len) > 1)
      ...
      for (a = 0; a < l->list.list_len - 1; a++)
	{
	  ...
	  if (p->expr_type == ET_EXPR_LITERAL_STRING && p2->expr_type == ET_EXPR_LITERAL_STRING)
	    {
...
usage_bottom_level (variable_usage * u)
{
  while (u->next)
    u = u->next;
  return u;
...
static char *
get_start_char_subscript (expr_str * e)
{
  struct variable_usage *u;
...
static char *
get_end_char_subscript (expr_str * e)
{
  struct variable_usage *u;
  static char buff[1024];
  switch (e->expr_type)
...
static char *
get_bottom_level_variable_name (expr_str * e)
{
 ...
 */
void
print_returning_g (int from_where, struct expr_str_list *bind, int allow_one_or_zero)
{
  int cnt;
  ...
  if (cnt)
	...
	{
	  printc
	    ("if (_retvars!= %d) {if (_retvars!=-1||1) {if (a4gl_status==0) A4GL_set_status(-3001,0);A4GL_pop_args(_retvars);}} else {A4GL_set_status(0,0);A4GL_pop_params(ibind,%d);}",
	     cnt, cnt);
	}
      else
	{
...
void
real_print_expr_list (struct expr_str_list *l)
{
  int a;
  l = A4GL_rationalize_list (l);
  if (l)
...
//
  ...
  if (fcall->expr_type == ET_EXPR_MEMBER_FCALL_NEW)
    {
      struct s_expr_member_function_call_n *p;
      int a;
      int nparam = 0;
      ...
      p = fcall->expr_str_u.expr_member_function_call_n;
      l = p->parameters;
      nparam = 0;
      if (l)
	{
  ...
  if (fcall->expr_type == ET_EXPR_GET_FLDBUF)
    {
      ...
      printc ("{");
      printc ("   int _retvars;");
      printc ("   _retvars=A4GL_fgl_getfldbuf(&_sio_%d,_inp_io_type,_fldlist, %s,NULL,0);", (int) p->sio_id,
  ...
  if (fcall->expr_type == ET_EXPR_PDF_FCALL)
    {
      ...
      if ( p->parameters &&  p->parameters->list.list_len) {
      	real_print_expr_list (p->parameters);
      	printc ("{int _retvars;A4GL_set_status(0,0);\n");
      	if (is_in_report ()) { printc ("_retvars=A4GL_pdf_pdffunc(&_rep,%s,%d);\n", p->functionname, p->parameters->list.list_len); }
      	else { printc ("_retvars=A4GL_pdf_pdffunc(NULL,%s,%d);\n", p->functionname, p->parameters->list.list_len); }
      } else {
      	printc ("{int _retvars;A4GL_set_status(0,0);\n");
      	if (is_in_report ()) { printc ("_retvars=A4GL_pdf_pdffunc(&_rep,%s,0);\n", p->functionname); }
      	else { printc ("_retvars=A4GL_pdf_pdffunc(NULL,%s,0);\n", p->functionname); }
      }
...
print_when (int has_expr, t_expr_str * expr)
{
  ...
  if (!has_expr)
    {				// We'd expect some form of test then...
	...
	case ET_EXPR_WORDWRAP:
	case ET_EXPR_CONCAT_LIST:
	  A4GL_warn ("Use of string for WHEN in a CASE with no expression");
	  break;
	...
	case ET_EXPR_TRUE:
	  A4GL_warn ("WHEN is always TRUE");
	  break;
	case ET_EXPR_FALSE:
	  A4GL_warn ("WHEN is always FALSE");
	  break;
...
void
print_init_var_from_expr (struct expr_str *v, char *prefix, int alvl, int explicit)
{
//
  A4GL_assertion (1, "FIXME");
}
...
void
print_event_actions (char *loopvar, struct on_events *events)
{
  int a;
  int n;
  if (events == 0)
...
//char comma=' ';
  ...
  if (n == 0)
      ...
      if (current_cmd->cmd_data.type == E_CMD_DISPLAY_ARRAY_CMD)
	{
      ...
      if (current_cmd->cmd_data.type == E_CMD_INPUT_CMD || current_cmd->cmd_data.type == E_CMD_CONSTRUCT_CMD
	  || current_cmd->cmd_data.type == E_CMD_INPUT_ARRAY_CMD)
	{
	  printc ("{-94,1,0,\"\"},");	/* EVENT_AFTER_INP_CLEAN */
	}
      printc (" {0}};");
  ...
  for (a = 0; a < n; a++)
	...
	case EVENT_ON_INTERVAL:
	  {
	    printh ("static long a4gl_idle%d=0;\n", idle_cnt);
	    printc ("{%d,%d,%d,(void *)&a4gl_idle%d},", A4GL_EVENT_ON_INTERVAL, a + 1, evt->evt_data.event_data_u.interval_n,
		    idle_cnt++);
	    break;
	  }
 ...
 */
int
print_bind_definition_g (struct expr_str_list *l, char i)
{

  switch (i)
    {
    case 'i':
      return print_bind_dir_definition_g (l, 0, i);
    case 'o':
      return print_bind_dir_definition_g (l, 0, i);
    case 'O':
      return print_bind_dir_definition_g (l, 0, i);
    ...
    case 'r':
      return print_bind_dir_definition_g (l, 0, i);
    default:
      A4GL_assertion (1, "Unhandled print_bind_definition");
    }
  return -1;
}
...
int
local_print_bind_set_value_g (struct expr_str_list *bind, int ignore_esqlc, int only_native, char type)
{
  int a;
  struct expr_str_list empty;
  char *start_chr;
...
int
print_bind_set_value_g (struct expr_str_list *bind, char type)
{
  return local_print_bind_set_value_g (bind, 0, 0, type);
}

char *
get_reset_state_after_call (int n)
{
  ...
  for (a = parent_stack_cnt - n; a >= 0; a--)
	...
	case E_CMD_DISPLAY_ARRAY_CMD:
	  ...
	  SPRINTF2 (buff,
		    "{char *_p; _p=A4GL_get_currwin_name(); A4GL_ensure_current_window_is(_curr_win); A4GL_reset_state_for(&_sio_%d,_sio_kw_%d);  A4GL_ensure_current_window_is(_p);}",
		    sio_id, sio_id);
	  return buff;
	...
	case E_CMD_CONSTRUCT_CMD:
	  ...
	  return buff;
	default:
	  break;		// the switch anyway, Keep going through the while - there might be something else...
...
void
print_reset_state_after_call (int n)
{
...
void
print_convert_report (char *repname, char *fname, char *otype, char *layout, char *file_or_pipe)
{
  ...
  printc ("A4GL_push_char(%s);A4GL_push_char(%s);A4GL_push_char(%s);%s%s(3,REPORT_CONVERT);", fname, otype, layout,
	  get_namespace (repname), repname);
}

void
print_free_convertible (char *repname)
{
...
//}
  ...
  if (scope == E_SCOPE_IMPORTED_GLOBAL && level == 0)
    {
      int skip = 0;
      if (strcasecmp (v->names.names.names_val[0].name, "time") == 0)
	skip++;
      if (strcasecmp (v->names.names.names_val[0].name, "pageno") == 0)
	skip++;
      if (strcasecmp (v->names.names.names_val[0].name, "lineno") == 0)
	skip++;
      if (strcasecmp (v->names.names.names_val[0].name, "today") == 0)
	skip++;
      if (strcasecmp (v->names.names.names_val[0].name, "usrtime") == 0)
	skip++;
      if (strcasecmp (v->names.names.names_val[0].name, "fgl_user") == 0)
	skip++;
  ...
  if (v->arr_subscripts.arr_subscripts_len)
	...
	{			// Dynamic array
	  ...
	  switch (nv->var_data.variable_type)
	    {
	    case VARIABLE_TYPE_SIMPLE:
	      if (nv->var_data.variable_data_u.v_simple.datatype == DTYPE_CHAR
		  || nv->var_data.variable_data_u.v_simple.datatype == DTYPE_VCHAR
...
int
local_print_agg_defines (char t, int a)
{
...
void
print_loaded_globals_file (char *s)
{
...
int
get_whenever_style (int code, char *whento)
{
...
static enum e_scope
local_find_variable_scope (expr_str * e)
{
...
int
check_binding (struct expr_str_list *l, char *fromwhere, int yylineno)
{
  char c;
  int a;
  if (l == 0)
	  ...
	  if (A4GL_isyes (acl_getenv ("WARNGLOBMODBINDING")))
	    {
	      if (c == E_SCOPE_MODULE)
		{
		  fprintf (stderr, "Use of module variable (%s) @%d (%s)\n", get_dbg_variable_name (l->list.list_val[a]), yylineno,
			   fromwhere);
		}
...
static void
dump_function_prototypes (module_definition * m)
{
  int a;
...
LEXLIB_A4GL_write_generated_code (struct module_definition *m)
{
  ...
  for (a = 0; a < m->module_entries.module_entries_len; a++)
	...
	case E_MET_IMPORT_FUNCTION_DEFINITION:
	  ...
	  print_import (m->module_entries.module_entries_val[a]->module_entry_u.import_function_definition.funcname,
  ...
  if (need_cursorname)
	...
	{
	  printc ("if (strcmp(s,\"%s\")==0) p=\"%s\";", m->clobberings.clobberings_val[a].important,
		  m->clobberings.clobberings_val[a].newval);
	}
...
char *
get_dbg_variable_name (expr_str * v)
{
...
set_get_subscript_as_string_top (struct variable_usage *u)
{
...
static int
subscript_range_check (void)
{
  if (do_subscript_range_check >= 0)
...
static struct variable *
set_get_subscript_as_string_next (struct variable *sgs_topvar, struct variable_usage *u)
{
  if (subscript_range_check () == 0)
    return NULL;
  if (sgs_topvar == NULL)
    return NULL;
...
char *
local_ident_as_string (expr_str * f, int quote)
{
  static char buff[2000];
  switch (f->expr_type)
    {
...
static void
print_variable_usage_gen (expr_str * v, int err_if_substring)
{
  struct variable_usage *u = 0;
  int substring = 0;
  expr_str *substring_start;
...
void
print_variable_usage (expr_str * v)
{
  print_variable_usage_gen (v, 1);
}

void
print_variable_usage_for_bind (expr_str * v)
{
...
void
print_push_variable_usage (expr_str * ptr)
{
  struct variable_usage *u;
  int substring = 0;
...
//char scope;
  ...
  if (!substring)
	...
	{
	case DTYPE_INT:
	  printc ("A4GL_push_long(");
	  print_variable_usage (ptr);
	  printc ("); ");
	  break;
	case DTYPE_SMINT:
	  printc ("A4GL_push_int(");
	  print_variable_usage (ptr);
	  printc (");");
	  break;
	case DTYPE_FLOAT:
	  printc ("A4GL_push_double(");
	  print_variable_usage (ptr);
	  printc (");");
	  break;
	case DTYPE_SMFLOAT:
	  printc ("A4GL_push_float(");
	  print_variable_usage (ptr);
	  printc (");");
	  break;
...
char *
local_expr_as_string (expr_str * s)
{
  static char rbuff[2000];
//char buff[2000];
  char buff_l[2000];
...
void
print_field_list (struct fh_field_list *flist)
{
  int a;
  set_nonewlines ();
...
void
remove_field_subscripts_from_field_list (struct fh_field_list *flist)
{
  int a;
  for (a = 0; a < flist->field_list_entries.field_list_entries_len; a++)
    {
...
struct variable *
local_find_variable_from_usage (struct variable_usage *u)
{
  struct variable *v = 0;
 ...
 */
void
print_fgllib_start (char *db, int is_schema, char *force_ui, char *debug_filename)
{
...
//
int
chk_ibind_select_internal (struct s_select *s)
{
  int a;
  int ok = 1;
...
int
chk_ibind_select (struct s_select *s)
{
  if (!chk_ibind_select_internal (s))
	...
	{
	  a4gl_yyerror
	    ("You can't SELECT a variable (perhaps you could rename the variable, or prefix the column in the SELECT list ?)");
	}
      else
	{
...
char *
escape_quotes_and_remove_nl (char *s)
{
