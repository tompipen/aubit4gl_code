...
# +----------------------------------------------------------------------+
#
# $Id: pack_packed.c,v 1.52 2013-07-12 13:00:31 mikeaubury Exp $
#*/
...
{
int a;
  a=A4GLPacker_output_long (s, len, 0, -1);
  return a;
...
int A4GLPacker_A4GL_valid_common_header(char* module,char* version) {
	...
	if (memcmp(buff,buff_r,strlen(buff))!=0) {
		if (A4GL_isyes(acl_getenv("IGNOREVERSIONS")))  { // Dont set this unless you're sure nothing serious has changed...
			return 1;
			// Ignore it - 
		}
		else {
