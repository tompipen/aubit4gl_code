...
l=NULL;
	...
	if (uses_serial) {
	if (!A4GL_isno(acl_getenv("SERIALWARN"))) { //@env SERIALWARN suppress the wasrning about serial columns being updated.
...
// Line counting ...
	...
	case E_CMD_SKIP_CMD:
		if (cmds->cmds.cmds_val[a]->cmd_data.command_data_u.skip_cmd.skip_amt->expr_type==ET_EXPR_LITERAL_LONG) {
			...
			a4gl_yyerror("You can't use a SKIP with an expression here - you must use  SKIP [INTEGER] LINE in headers/trailers");
