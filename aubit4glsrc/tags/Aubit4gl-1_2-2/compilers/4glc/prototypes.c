...
static void add_rettypes(int funcno, struct rettypes *rl) {
	int a;
	int found=1;
	...
	functions[funcno].return_datatypes=realloc(functions[funcno].return_datatypes, sizeof(functions[funcno].return_datatypes[0])*functions[funcno].nreturn_datatypes);
	functions[funcno].return_datatypes[functions[funcno].nreturn_datatypes-1]=rl;

}

static struct rettypes * prototype_returns(int a,command *c) {
	struct rettypes *rl;
	int d;
...
static void process_return_types_for_function(int a) {
int b;
			...
			if (functions[a].nparameters) {
				for (b=0;b<functions[a].nparameters;b++) {
					fprintf(output,"   <EXPR NO=\"%d\" DATATYPE=\"%d\" LENGTH=\"%d\" NAME=\"%s\"/>\n", 
						b,
						functions[a].parameters[b]&DTYPE_MASK,
						functions[a].parameters[b]>>16,
...
int a;
  ...
  if (is_bolton_function (funcname)!=-1 ) {
		return bolton_function_datatype(funcname);
	}
  ...
  for (a=0;a<functions_cnt;a++) {
	if (strcmp(functions[a].function,funcname)==0) {
		...
		if (functions[a].nreturn_datatypes==1) { // A single return
			if ( functions[a].return_datatypes[0]->nreturns==1) { // OF a single value..
				return functions[a].return_datatypes[0]->returns[0];
			}
		} else {
			// more than one return...
			for (b=0;b<functions[a].nreturn_datatypes;b++) {
				if ( functions[a].return_datatypes[b]->nreturns==1) { // Does it have 1 value ? 
					if (functions[a].return_datatypes[b]->returns[0]!=-1) { // Is it sensible ? 
		...
		if (functions[a].processed_returns==1) return -1;
