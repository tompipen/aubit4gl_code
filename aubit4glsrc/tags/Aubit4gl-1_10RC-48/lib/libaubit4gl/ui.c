 ...
 */
void
A4GL_disp_fields (int n, int attr, ...)
{
  va_list ap;
  va_start (ap, attr);
 ...
 */
int
A4GL_disp_form_fields (int n, int attr, char *formname, ...)
{
  va_list ap;
  int a;
 ...
 */
int
A4GL_req_field (void *s, char itype, char type, ...)
{
  va_list ap;
  int a = 0;
  int found = 0;
 ...
 */
int
A4GL_gen_field_chars (void ***field_list, void *formdets, ...)
{
  va_list ap;
  int a;
  ...
  if (strlen (s))
      ...
      if (clr_end_of_line && A4GL_was_last_empty () && n == 1 && (tos_dtype & 0xff) == 0 && tos_size == 0)
	{
	  if (x >= 0 && y >= 0)
	    ...
	    A4GL_display_internal (x, y, "", a, 0);	// Line mode
...
int
A4GL_pushint_fgl_infield (void *inp, char itype, ...)
{
  int a = -1;
  va_list ap;
...
int
A4GL_fgl_infield (void *inp, char itype, ...)
{
  int a = -1;
  va_list ap;
...
int
A4GL_open_gui_form (long *form_variable, char *name_orig, int absolute, int nat, char *like, int disable, void *handler_e,
 ...
 */
int
A4GL_fgl_fieldtouched (void *input, char itype, ...)
{
  int a = -1;
  va_list ap;
...
char *
A4GL_pull_off_data_for_display (int n, int display_type)
{
  int z;
  char *s = 0;
  ...
  for (z = n - 1; z >= 0; z--)
      ...
      if (A4GL_has_datatype_function_i (tos_dtype & DTYPE_MASK, "DISPLAY"))
	{
	  ...
	  ptr = function (tos_ptr, tos_size, -1, (struct struct_scr_field *) 0, display_type);
...
int
A4GL_field_name_str_match (char *f1, char *f2)
{
  int a;
  char *f1p;
  char *f2p;
...
void
A4GL_chk_for_screen_print (int a)
{
...
void
A4GL_add_key_mapping (int src, int dest)
{
  nkeymappings++;
...
int
aclfgl_aclfgl_add_keymap (int n)
{
  int dest;
  int src;
  dest = A4GL_pop_long ();
...
int
A4GL_has_event_for_keypress (int a, struct aclfgl_event_list *evt)
{
  int n;
  for (n = 0; evt[n].event_type; n++)
    {
      if (evt[n].event_type == -90 && evt[n].keycode == a)
	return evt[n].block;
    }
  return 0;
}
...
int
A4GL_has_event_for_action (char *a, struct aclfgl_event_list *evt)
{
  int n;
  int cde;
  for (n = 0; evt[n].event_type; n++)
...
char *
A4GL_find_gui_id_name_from_id (int id)
{
  if (id < 0 || id >= gui_id_names_cnt)
    return 0;
  return gui_id_names[id];
}

int
A4GL_find_gui_id_from_name (char *s)
{
  int a;
  if (gui_id_names_cnt == 0)
    return -1;

  for (a = 0; a < gui_id_names_cnt; a++)
    {
      if (strcmp (gui_id_names[a], s) == 0)
	return a;
    }
  return -1;
}
...
void
A4GL_set_line_mode_column (int n, int ab_rel)
{
  if (ab_rel == 0)
    {
      line_mode_column = n;
...
char *
A4GL_linemode_goto_column (int a)
{
  int b;
...
int
aclfgl_aclfgl_get_curr_height (int n)
{
...
int
A4GL_has_evt_timeout (struct aclfgl_event_list *evt)
{
  int a;
  long now;
  long then;
...
void
A4GL_evt_not_idle (struct aclfgl_event_list *evt)
{
  int a;
  long now;
  now = time (0);
...
void
A4GL_add_recall_value (char *field_name, char *value)
{
  ...
  if (s->first)
    {
      e = s->first;
      if (strcmp (e->recall_value, value) == 0)
	{			// Its already at the front of the list..
	  return;
	}
    }
  ...
  while (e)
    {
      ...
      n = e->next;
      if (strcmp (value, e->recall_value) == 0)
	{			// It was already in our list
...
void
A4GL_debug_dump_recall (char *field_name)
{
  struct s_recall_list *s;
  struct s_recall_entry *e;
...
char *
A4GL_recall_field (char *t, char *c, int x, int y, int show)
{
  struct s_recall_list *s;
  struct s_recall_entry *e;
  int maxlen = 0;
  ...
  if (nvals > 100)
    nvals = 100;		// Gotta call it sometime...
  ...
  while (1)
      ...
      if (key == A4GLKEY_F (12))
	{
      ...
      if (key == A4GLKEY_ENTER)
	{
	  ok = 1;
	  break;
	}
      if (key == A4GLKEY_UP && crow > 0)
	{
	  crow = crow - 1;
	  sline--;
	}
      if (key == A4GLKEY_DOWN && crow < (nvals - 1))
	{
	  crow = crow + 1;
	  sline++;
	}
  ...
  if (ok)
    {
      last_val = acl_strdup (values[crow]);
    }
...
A4GL_include_range_check (char *ss, char *ptr, int dtype)
{
  ...
  if (ptr3 == 0)
      ...
      if (A4GL_aubit_strcasecmp (ptr2, "NULL") == 0)
	{
      ...
      if ((dtype == DTYPE_SMINT || dtype == DTYPE_INT || dtype == DTYPE_DECIMAL || dtype == DTYPE_FLOAT || dtype == DTYPE_SMFLOAT)
...
void
A4GL_set_fcompile_err (char *s)
{
  strcpy (last_fcompile_err, s);
}

char *
A4GL_get_fcompile_err (void)
{
...
char *
A4GL_show_menu_large_get_matches (ACL_Menu * menu, char *typed_portion, int width, int *pcnt /* return number of matches */ ,
				  ACL_Menu_Opts ** uniq)
{
      ...
      if (!opt1->attributes & ACL_MN_HIDE)
	{
	  ...
	  if (A4GL_aubit_strcasecmp (typed_portion, buff_opt) == 0)
	    {			// We match!
	      ...
	      if (!elipses)
		{		// Have we already reached our limit ?
		  if (opt1->opt_title[0] == ' ')
		    {
		      strcpy (ldisp, disp);
...
void
aclfgl_key_callback (void *ptr, int keyCode)
{
  int a;
...
int
A4GL_is_event_close (void *event)
{
  return 0;
}
