...
# +----------------------------------------------------------------------+
#
# $Id: data_if.c,v 1.44 2015-01-27 10:19:47 mikeaubury Exp $
#
*/
...
get_set_s_screenio (void *ptr, int mode, char *name, long var)
{
  ...
  if (strcmp (name, "callback_function") == 0)
    {
      if (mode == GETSETGET)
	return (void *) val->callback_function;
      if (mode == GETSETGETPTR)
	return (void *) &(val->callback_function);
      if (mode == GETSETSET)
	{
	  val->callback_function = (void *) var;
	  return (void *) 1;
	}
    }
...
get_set_s_inp_arr (void *ptr, int mode, char *name, long var)
{
  ...
  if (strcmp (name, "current_row_display") == 0)
    {
      if (mode == GETSETGET)
        return (void *) (long) val->current_row_display;
      if (mode == GETSETGETPTR)
        return (void *) &(val->current_row_display);
      if (mode == GETSETSET)
        {
          val->current_row_display = (int) var;
          return (void *) 1;
        }
    }
...
int
A4GL_get_set_as_int (char *str, void *ptr, int mode, char *name, long var)
{
  int a;
