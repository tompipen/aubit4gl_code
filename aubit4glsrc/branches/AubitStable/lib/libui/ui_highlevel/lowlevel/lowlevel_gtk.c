...
static char const module_id[] =
  "$Id: lowlevel_gtk.c,v 1.83.2.1 2006-03-24 17:24:30 mikeaubury Exp $";
#endif
...
int A4GL_keypress (GtkWidget * widget, GdkEventKey * event,
		   gpointer user_data);
...
A4GL_LL_create_window (int h, int w, int y, int x, int border)
{
  ...
  if (x == 0 && y == 0 && h == 0 && w == 0)
      ...
      else
	{
      ...
      gtk_button_box_set_layout (GTK_BUTTON_BOX (fglmenu_bb),
				 GTK_BUTTONBOX_START);
      gtk_button_box_set_layout (GTK_BUTTON_BOX (ok_cancel),
				 GTK_BUTTONBOX_START);
	...
	{
	  gtk_box_set_child_packing (GTK_BOX (hbox), frame, 1, 1, 2,
				     GTK_PACK_START);
	}
      else
	{
...
A4GL_LL_int_form_driver (void *vform, int mode)
{
  ...
  if (mode <= 255 && a_isprint (mode) && mode >= ' ')
      ...
      if (form->ovlins == 1)
	{
	  ...
	  form->curcol++;
	  if (form->curcol >= m && autonext) form->curcol = 0;
	}
      else
	{
	  //printf("overwrite\n");
